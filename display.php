...
// Now let's include all the sub functions
require_once __DIR__ . '/genre.php';
require_once __DIR__ . '/artist.php';
require_once __DIR__ . '/head-foot.php';
 ...
 *
 * @param The   $numAlbums number of albums covers to show
 * @param mixed $level
 ...
 * @version 04/29/04
 */
function displayRandomAlbums($numAlbums, $level)
{
    global $web_root, $root_dir, $media_dir, $album_name_truncate, $this_page, $url_seperator, $artist_truncate, $row_colors, $word_play, $word_play_random, $random_rate, $random_per_slot;

    // Let's read all the possible albums

    $retArray = readDirInfo($web_root . $root_dir . '/data/tracks', 'file');

    // Now let's see if we need to filter

    if ('' != $level) {
        for ($ctr = 0, $ctrMax = count($retArray); $ctr < $ctrMax; $ctr++) {
            if (mb_stristr(mb_strtolower($retArray[$ctr]), mb_strtolower($level))) {
                $retArray2[] = $retArray[$ctr];
            }
        }
    ...
    // Now let's shuffle and slice

    shuffle($retArray);

    shuffle($retArray);

    $retArray = array_slice($retArray, 0, ($numAlbums * 25));

    $c = 0;

    // Now let's get the album images

    for ($ctr = 0, $ctrMax = count($retArray); $ctr < $ctrMax; $ctr++) {
        // Now let's grab the first one as our XML file

        $xmlFile = $web_root . $root_dir . '/data/tracks/' . $retArray[$ctr];

        if (mb_stristr($xmlFile, 'NULL')) {
            continue;
        }
        ...
        $searchVals = 'genre|artist|album|image|';
        ...
        for ($e = 0, $eMax = count($searchArray); $e < $eMax; $e++) {
            $$searchArray[$e] = str_replace(
                ...
                str_replace(
                    ...
                    mb_substr(
                        $contents,
                        mb_strpos($contents, '<' . $searchArray[$e] . '>') + mb_strlen('<' . $searchArray[$e] . '>'),
                        mb_strpos($contents, '</' . $searchArray[$e] . '>') - (mb_strpos($contents, '<' . $searchArray[$e] . '>') + mb_strlen('<' . $searchArray[$e] . '>'))
                    )
                )
            );
        }

        // Now let's verify that the image actually exists

        $image = str_replace($web_root, '', $image);

        if (is_file($web_root . urldecode($image)) and '' != $image) {
            // Now let's make sure the directory exists where we expect it

            if (is_dir($web_root . $root_dir . $media_dir . '/' . $genre . '/' . $artist . '/' . $album)) {
                $imgArray[$c]['image'] = $image;

                $imgArray[$c]['genre'] = $genre;

                $imgArray[$c]['artist'] = $artist;

                $imgArray[$c]['album'] = $album;
    ...
    // Now let's slice again, but increase by 4 so we'll have plenty to rotate

    if (!isset($imgArray)) {
        return;
    }

    $imgArray = array_slice($imgArray, 0, ($numAlbums * ($random_per_slot * 2)));

    // Now let's make sure we got enough images to satisfy what we need below

    if (count($imgArray) < ($numAlbums * $random_per_slot)) {
        // Let's figure out if we cut the slots of the amounts per slot

        if (count($imgArray) < $numAlbums) {
            // Ok, we need to cut the slots

            $numAlbums = round(($numAlbums / $random_per_slot) - .49);
        } else {
            // Ok, let's cut the amount per slot

            $random_per_slot = round(($random_per_slot / count($imgArray)) + .49);
        }
    }

    // Now let's figure out how wide to make the colums

    $colWidth = 100 / $numAlbums;
    ...
    echo '<strong>' . word_random_albums;
    ...
    while ($c < ($numAlbums + 1)) {
        jzTDOpen($colWidth, 'center', 'middle', 'jz_track_table_songs_td', '');
    ...
    foreach ($imgArray as $filename) {
        $albumLink = $this_page . $url_seperator . 'ptype=artist&genre=' . urlencode($filename['genre']) . '&artist=' . urlencode($filename['artist']) . '&album=' . urlencode($filename['album']);

        $albumName = returnItemShortName($filename['album'], 12);

        $artistLink = $this_page . $url_seperator . 'ptype=artist&genre=' . urlencode($filename['genre']) . '&artist=' . urlencode($filename['artist']);

        $artistName = returnItemShortName($filename['artist'], 12);

        $imgSrc = "<img height='100' title='" . $filename['artist'] . ' - ' . $filename['album'] . "' src='" . str_replace('%2F', '/', rawurlencode($filename['image'])) . "' border='0'>";

        $item_link = $this_page . $url_seperator . 'ptype=songs&genre=' . urlencode($filename['genre']) . '&artist=' . urlencode($filename['artist']) . '&album=' . urlencode($filename['album']);

        $playLink = $root_dir . '/playlists.php?d=1&style=normal&info=' . urlencode($filename['genre']) . '/' . urlencode($filename['artist']) . '/' . urlencode($filename['album']);

        //$word_play

        $randLink = $root_dir . '/playlists.php?d=1&style=random&info=' . urlencode($filename['genre']) . '/' . urlencode($filename['artist']) . '/' . urlencode($filename['album']);
        ...
        //echo "imgItem[". $c. "]=\"". $albumName. "<br>". $artistName. "<br><a href='". $item_link. "'>". $imgSrc. "</a><br><a href='". $playLink. "'>". $word_play. "</a> - <a href='". $randLink. "'>". $word_play_random. "</a>\"". "\n";
            //$c++;
    }
