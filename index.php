...
if (isset($_POST['submitAction']) or isset($_POST['albumSubmitAction'])) {
    ...
    switch ($submitAction) {
        case 'play': # Ok, they just wanted to play what was selected
            playSelected();
            break;
        case 'playrandom': # Ok, they just wanted to play what was selected
            playSelected(true);
            break;
...
if (isset($_POST['genre'])) {
    header("Location: $this_page" . $url_seperator . 'ptype=' . $_POST['genre']);
}
if (isset($_POST['artist'])) {
    header("Location: $this_page" . $url_seperator . 'ptype=' . $_POST['artist']);
}
if (isset($_POST['album'])) {
    header("Location: $this_page" . $url_seperator . 'ptype=' . $_POST['album']);
}
if (isset($_POST['previously_viewed'])) {
    header("Location: $this_page" . $url_seperator . 'ptype=' . $_POST['previously_viewed']);
}
...
switch ($pageType) {
    ...
    case 'tools': # They want the tools page (ID3 tags, etc)
        ...
        break;
    case 'search': # They want the search page
        if ('true' == $cms_mode) {
            openCMS();
        }
        require_once __DIR__ . '/search.php';
