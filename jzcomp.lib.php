...
class _jzRessource
{
    ...
    {
        return false !== $this->cur_file_handle;
    }

    public function Close()
    {
        if ($this->isOpen()) {
            fclose($this->cur_file_handle);
        }
    ...
    public function OpenNext()
    {
        ...
        if (false !== ($this->cur_file_handle = @fopen($this->files[0], 'rb'))) {
            return $this->files[0];
        }
...
class jzStreamRaw
{
     ...
     *
     * @param int $size size of data to read, rounded to 512 bytes.
...
class jzStreamTar
{
    ...
    public function Read($size)
    {
        ...
        if (!$this->_ressource->isOpen()) {
            ...
            $v_binary_data .= self::_tar_header(basename($filename), $filename);
        }
        ...
        for ($i = 0; $i < $size / 512; $i++) {
            if ('' != ($v_buffer = $this->_ressource->getData(512))) {
                ...
                if (false !== ($filename = $this->_ressource->OpenNext())) {
                    $v_binary_data .= self::_tar_header(basename($filename), $filename);
    ...
    public function _tar_header($archive_filename, $real_filename)
    {
        $archive_filename = mb_substr($archive_filename, 0, 99);
...
class jzStreamZip
{
            ...
            if ('' != ($v_buffer = $this->_ressource->getData(512))) {
                ...
                if (false !== ($filename = $this->_ressource->OpenNext())) {
                    ...
                    $v_binary_data .= $ctrl_dir .        // all the central directory at once
                                      ...
                                      pack('v', count($this->_ctrl_dir)) .     // total # of entries "on this disk"
                                      pack('v', count($this->_ctrl_dir)) .     // total # of entries overall
                                      pack('V', mb_strlen($ctrl_dir)) .             // size of central dir
    ...
    public $_ctrl_dir = [];    // central directory
    public $_old_offset = 0;        // offset of next local header
    public $_new_offset = 0;        // next offset of next local header
    public $_cur_crc = 0;            // current file crc
    public $_cur_size = 0;            // current file size
    public $_cur_name = '';        // current file name
    ...
    public function _zip_header($name)
    {
        // get its crc & size
        ...
        $time = ((int)$expd[3]) << 11 | ((int)$expd[4]) << 5;

        $date = (((int)$expd[0]) - 1980) << 9 | ((int)$expd[1]) << 5 | ((int)$expd[2]);
        ...
        $header .= pack('v', mb_strlen($name)); //length of filename
        $header .= pack('v', 0); //extra field length
        $header .= $name;

        // update current writing offset

        $this->_new_offset += mb_strlen($header);
    ...
    public function _zip_footer()
    {
        ...
        $this->_ctrl_dir[$file_nb] .= pack('v', mb_strlen($this->_cur_name)); //length of filename
