...
function displayHeader($title, $endBreak = 'true')
{
    ...
    // Now let's set what they are viewing so we'll know the last few that have been viewed

    setViewing(jzstripslashes($genre . '/' . $artist . '/' . $album));
        ...
        switch ($_GET['editmenu']) {
            case 'album':
                ...
                <SCRIPT LANGUAGE=JAVASCRIPT TYPE="TEXT/JAVASCRIPT"><!--\
						openPopup("<?php echo $root_dir . '/mp3info.php?type=album&info=' . urlencode($genre) . '/' . urlencode($artist) . '/' . urlencode($album) . '&cur_theme=' . $_SESSION['cur_theme']; ?>", 320, 600);
                    history.back();
                    -->
                </SCRIPT>
                <?php
            ...
            case 'artist':
                ...
                ?>
                <SCRIPT LANGUAGE=JAVASCRIPT TYPE="TEXT/JAVASCRIPT"><!--\
						openPopup("<?php echo $root_dir . '/mp3info.php?type=artist&info=' . urlencode($genre) . '/' . urlencode($artist) . '&cur_theme=' . $_SESSION['cur_theme']; ?>", 320, 600);
                    history.back();
                    -->
                </SCRIPT>
                <?php
            ...
            case 'bulkalbum':
                ...
                ?>
                <SCRIPT LANGUAGE=JAVASCRIPT TYPE="TEXT/JAVASCRIPT"><!--\
						openPopup("<?php echo $root_dir . '/mp3info.php?type=bulkalbum&info=' . urlencode($genre) . '/' . urlencode($artist) . '/' . urlencode($album) . '&cur_theme=' . $_SESSION['cur_theme']; ?>", 320, 400);
                    history.back();
                    -->
                </SCRIPT>
                <?php
            ...
            case 'addfake':
                ...
                ?>
                <SCRIPT LANGUAGE=JAVASCRIPT TYPE="TEXT/JAVASCRIPT"><!--\
						openPopup("<?php echo $root_dir . '/mp3info.php?type=addfake&info=' . urlencode($genre) . '/' . urlencode($artist) . '/' . urlencode($album) . '&cur_theme=' . $_SESSION['cur_theme']; ?>", 320, 600);
                    history.back();
                    -->
                </SCRIPT>
                <?php
            ...
            case 'addmedia':
                ...
                ?>
                <SCRIPT LANGUAGE=JAVASCRIPT TYPE="TEXT/JAVASCRIPT"><!--\
						openPopup("<?php echo $root_dir . '/mp3info.php?type=addmedia&uploadDest=' . urlencode($_GET['info']) . '&cur_theme=' . $_SESSION['cur_theme']; ?>", 550, 400);
                    history.back();
                    -->
                </SCRIPT>
                <?php
            ...
            case 'rate':
                ...
                ?>
                <SCRIPT LANGUAGE=JAVASCRIPT TYPE="TEXT/JAVASCRIPT"><!--\
						openPopup("<?php echo $root_dir . '/mp3info.php?type=rate&info=' . urlencode($_GET['info']) . '&cur_theme=' . $_SESSION['cur_theme']; ?>", 300, 150);
                    history.back();
                    -->
                </SCRIPT>
                <?php
            ...
            case 'discuss':
                ...
                ?>
                <SCRIPT LANGUAGE=JAVASCRIPT TYPE="TEXT/JAVASCRIPT"><!--\
						openPopup("<?php echo $root_dir . '/mp3info.php?type=discuss&info=' . urlencode($_GET['info']) . '&cur_theme=' . $_SESSION['cur_theme']; ?>", 500, 400);
                    history.back();
                    -->
                </SCRIPT>
                <?php
    ...
    echo '<span style="font-size:0px;">.</span></nobr>';

    // Now let's see if they get the tools menu

    if ('admin' == $_SESSION['jz_access_level']) {
        global $jinzora_skin, $word_update_cache, $word_upload_center, $word_update_id3v1, $word_user_manager, $word_search_for_album_art, $word_enter_setup, $word_check_for_update, $word_add_fake_track, $jz_MenuItemLeft, $jz_MenuSplit, $jz_MenuItemHover, $jz_MainItemHover, $main_img_dir, $jz_MenuItem;

        require_once __DIR__ . '/tools-menu.php';
    }
    ...
    if ('true' == $header_drops and 'noaccess' != $_SESSION['jz_access_level']) {
        ...
        if ('true' == $genre_drop) {
            jzTDOpen('15', '', 'top', 'jz_header_table_td', '');
            ...
            echo '<select name="genre" onChange="submit()" class="jz_select" style="width:150;">';
            ...
            for ($ctr = 0, $ctrMax = count($genreArray); $ctr < $ctrMax; $ctr++) {
                if ('' != $genreArray[$ctr]) {
                    ...
                    switch ($directory_level) {
                        case '3': # 3 directories deep
                            echo '<option value="genre&genre=' . urlencode($genreArray[$ctr]) . '">' . $title . '</option>' . "\n";
                            break;
                        case '2': # 2 directories deep
                            echo '<option value="artist&genre=/&artist=' . urlencode($genreArray[$ctr]) . '">' . $title . '</option>' . "\n";
                            break;
                        case '1': # 1 directories deep
                            echo '<option value="songs&genre=/&artist=/&album=' . urlencode($genreArray[$ctr]) . '">' . $title . '</option>' . "\n";
                            break;
                    }
                }
            }
        ...
        if (('3' == $directory_level or '2' == $directory_level) and 'true' == $artist_drop) {
            ...
            echo '<select name="artist" onChange="submit()" class="jz_select" style="width:150;">';
            ...
            for ($ctr = 0, $ctrMax = count($artistArray); $ctr < $ctrMax; $ctr++) {
                if ('' != $artistArray[$ctr]) {
                    ...
                    switch ($directory_level) {
                        case '3': # 3 directories deep
                            echo '<option value="artist&genre=' . urlencode($artistDisplay[1]) . '&artist=' . urlencode($artistDisplay[0]) . '">' . $title . '</option>' . "\n";
                            break;
                        case '2': # 2 directories deep
                            echo '<option value="songs&genre=/&artist=' . urlencode($artistDisplay[1]) . '&album=' . urlencode($artistDisplay[0]) . '">' . $title . '</option><br>';
                            break;
                    }
                }
            }
        ...
        if ('3' == $directory_level and 'true' == $album_drop) {
            ...
            echo '<select name="album" onChange="submit()" class="jz_select" style="width:150;">';
            ...
            for ($ctr = 0, $ctrMax = count($albumArray); $ctr < $ctrMax; $ctr++) {
                if ('' != $albumArray[$ctr]) {
                    ...
                    echo '<option value="songs&genre=' . urlencode($albumDisplay[1]) . '&artist=' . urlencode($albumDisplay[2]) . '&album=' . urlencode($albumDisplay[0]) . '">' . $title . '</option>' . "\n";
                }
            }
            ...
            for ($ctr = 0, $ctrMax = count($songArray); $ctr < $ctrMax; $ctr++) {
                if ('' != $songArray[$ctr]) {
                    ...
                    if (preg_match("/\.($audio_types)$/i", $title) || preg_match("/\.($video_types)$/i", $title)) {
                        ...
                        echo '<option value="quickplay&style=normal&qptype=song&info=' . $info . '">' . $title . '</option>' . "\n";
                    }
                }
            }
        ...
        if ('' == $_GET['ptype'] or 'genre' == $_GET['ptype']) {
        ...
        // Now let's see if they wanted more

        if ('true' == $display_previous or 'true' == $display_recommended or ('0' != $echocloud or '' != $echocloud and $directory_level = '3')) {
            jzTROpen();

            if ('' != $_GET['ptype'] and 'genre' != $_GET['ptype']) {
                displayPreviousAlbums();
            }

            // Now let's show the Echocloud similar artists

            if ('0' != $echocloud and '' != $echocloud and '3' == $directory_level) {
                // Let's make sure they are on the Artists page
                ...
                if (isset($_GET['ptype'])) {
                    if ('artist' == $_GET['ptype'] or 'songs' == $_GET['ptype']) {
                        ...
                        if ($fp) {
                            $path = '/psearch.php?searchword=' . rawurlencode($artist) . '&nrows=25';
                            ...
                            if (count($ecArray) > 1) {
                                ...
                                echo '<span class="jz_header_table_href">' . word_similiar_artists . '</span>';

                                echo '<br>';

                                echo '<form action="' . $this_page . '" method="post">';

                                echo '<select name="previously_viewed" class="jz_select" onChange="submit()" style="width:150;">';
                            ...
                            for ($i = 0, $iMax = count($ecArray); $i < $iMax; $i++) {
                                if ('' != $ecArray[$i]) {
                                    ...
                                    if ($ecArtist != $artist) {
                                        // Now let's make sure we have that artist

                                        if (mb_stristr(mb_strtolower(urldecode($_SESSION['artist_list'])), mb_strtolower($ecArtist))) {
                                            // Now let's break out the data

                                            $artistArray = explode("\n", $_SESSION['artist_list']);

                                            for ($e = 0, $eMax = count($artistArray); $e < $eMax; $e++) {
                                                // Now let's make sure they have this artist

                                                if (mb_stristr(mb_strtolower(urldecode($artistArray[$e])), mb_strtolower($ecArtist))) {
                                                    $dataArray = explode('--', $artistArray[$e]);

                                                    $optVal = 'artist&genre=' . $dataArray[1] . '&artist=' . $dataArray[0];
                                                }
                                            }

                                            // Now let's make sure that's not who we're looking at

                                            if (!mb_stristr(mb_strtolower($artist), mb_strtolower($ecArtist))) {
                                                $artSugArray[] = $dataArray[1] . '/' . $dataArray[0];

                                                echo '<option value="' . $optVal . '">' . ucwords(mb_strtolower($ecArtist)) . '</option>' . "\n";
                                            }
                                        } else {
                                            // Ok they did NOT have this artist, let's track it for later

                                            if (!mb_stristr($nonMatch, ucwords(mb_strtolower($ecArtist)))) {
                                                $nonMatch .= '-|-' . ucwords(mb_strtolower($ecArtist));
                                            }
                                        }
                                    }
                                }
                        ...
                        // Now let's see if there were artists that we found that didn't match ones we had

                        if ($nonMatch) {
                            echo '<option value="artist&genre=' . urlencode($genre) . '&artist=' . urlencode($artist) . '"></option>' . "\n";

                            echo '<option value="artist&genre=' . urlencode($genre) . '&artist=' . urlencode($artist) . '">------------------------------</option>' . "\n";

                            $nonMatchArray = explode('-|-', $nonMatch);

                            for ($e = 0, $eMax = count($nonMatchArray); $e < $eMax; $e++) {
                                if ('' != $nonMatchArray[$e]) {
                                    $optVal = 'artist&genre=' . urlencode($genre) . '&artist=' . urlencode($artist) . '&search=amg&type=artists&info=' . urlencode($nonMatchArray[$e]);

                                    echo '<option value="' . $optVal . '">' . $nonMatchArray[$e] . '</option>' . "\n";
                                }
                            }
                        }
                ...
                if ('true' == $display_recommended and isset($artSugArray)) {
                    if (0 != count($artSugArray)) {
                        // Now let's shuffle it and slice it

                        shuffle($artSugArray);

                        $artSugArray = array_slice($artSugArray, 0, 5);

                        // Now let's look at the artists we found, and get some albums from them

                        for ($e = 0, $eMax = count($artSugArray); $e < $eMax; $e++) {
                            $compArray = explode('/', $artSugArray[$e]);

                            $comp1 = mb_strtolower(urldecode($compArray[1]));

                            $comp2 = mb_strtolower(urldecode($compArray[0]));

                            for ($i = 0, $iMax = count($albumArray); $i < $iMax; $i++) {
                                if (mb_stristr(mb_strtolower($albumArray[$i]), $comp1) or mb_stristr(mb_strtolower($albumArray[$i]), $comp0)) {
                                    $compMatch[] = $albumArray[$i];
                        ...
                        if (isset($compMatch)) {
                            if (0 != count($compMatch)) {
                                // Now let's show the drop down

                                jzTDOpen('15', 'left', 'top', 'jz_header_table_td', '0');

                                echo '<nobr>';

                                echo '<span class="jz_header_table_href">' . word_similar_albums . '</span>';
                                ...
                                for ($i = 0, $iMax = count($compMatch); $i < $iMax; $i++) {
                                    // Now let's break it out

                                    $albumDisplay = explode('--', $compMatch[$i]);

                                    $title = returnItemShortName($albumDisplay[2] . ' -- ' . $albumDisplay[0], $quick_list_truncate);

                                    echo '<option value="songs&genre=' . urlencode($albumDisplay[1]) . '&artist=' . urlencode($albumDisplay[2]) . '&album=' . urlencode($albumDisplay[0]) . '">' . ucwords($title) . '</option>' . "\n";
                                }
...
function displayPreviousAlbums()
{
    global $display_previous, $this_page, $word_pleasechoose, $directory_level;

    // Now let's display the previously viewed stuff IF they want it

    if ('true' == $display_previous) {
        jzTDOpen('15', 'left', 'top', 'jz_header_table_td', '0');

        echo '<nobr>';

        echo '<span class="jz_header_table_href">' . word_previously_viewed . '</span>';
        ...
        for ($c = 0, $cMax = count($optArray); $c < $cMax; $c++) {
            ...
            // Now let's set the display

            $valArray = explode('---', $optArray[$c]);

            switch ($directory_level) {
                case '3':
                    $dispVal = $valArray[count($valArray) - 1] . ' - ' . $valArray[count($valArray) - 2];
                    //$dispVal = returnItemShortName($dispVal,$quick_list_truncate);
                    $optVal = 'songs&genre=' . urlencode($valArray[0]) . '&artist=' . urlencode($valArray[1]) . '&album=' . urlencode($valArray[2]);
                    break;
            }

            // Now let's make sure we're not looking at this item

            if (isset($_GET['ptype'])) {
                if ('songs' == $_GET['ptype']) {
                    if (mb_stristr(mb_strtolower(urldecode($_GET['album'])), mb_strtolower($valArray[2]))) {
                        $dispVal = '';
                    }
                }
            }
            ...
            if ('' != $dispVal) {
                echo '<option value="' . $optVal . '">' . $dispVal . '</option>' . "\n";
            }
        }
