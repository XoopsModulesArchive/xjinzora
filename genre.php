...
function displayGenre()
{
    ...
    // Now let's look and see if there are any sub folders here

    // If not we really should be looking at the song view instead

    if (0 == returnSubItems($genre)) {
        echo '<META HTTP-EQUIV="Refresh" CONTENT="0; URL=' . $this_page . $url_seperator . 'ptype=songs&genre=&artist=&album=' . urlencode($_GET['genre']) . '">' . $css . '<center>' . $word_please_wait . '</center>';
    ...
    if ('' != $genre and '/' != $genre) {
        displayHeader($genre . returnSubNumber($genre, 'artist'));
    } else {
        switch ($directory_level) {
            ...
            case '2': # 2 directories deep
                displayHeader($word_artist);
                break;
            case '1': # 2 directories deep
                displayHeader($word_album);
                break;
        }
    }
    ...
    if ('' == $genre) {
        $retArray = returnAllGenres();
    } else {
        $retArray = returnSelectedArtists($genre);
    }
    ...
    for ($lctr = 0, $lctrMax = count($retArray); $lctr < $lctrMax; $lctr++) {
        ...
        $displayItem = returnItemShortName($retArray[$lctr], $artist_truncate);

        // Now let's get the number of sub items

        $fldr_ctr = returnSubNumber($retArray[$lctr]);
        ...
        $link_url = urlencode($genre) . '/' . urlencode($retArray[$lctr]);
        ...
        if ('viewonly' != $_SESSION['jz_access_level'] and 'lofi' != $_SESSION['jz_access_level']) {
            ...
            if ('all' == $info_level or ('admin' == $info_level and 'admin' == $_SESSION['jz_access_level'])) {
                ...
                $item_url = $root_dir . '/mp3info.php?type=' . $ptype . '&info=' . urlencode($genre) . '/' . urlencode($retArray[$lctr]) . '&cur_theme=' . $_SESSION['cur_theme'];
            ...
            if ('true' == $enable_ratings) {
                ...
                $item_url = $root_dir . '/mp3info.php?type=rate&info=' . rawurlencode($genre) . '/' . rawurlencode($retArray[$lctr]) . '&cur_theme=' . $_SESSION['cur_theme'];
            ...
            if ('true' == $enable_discussion) {
                ...
                $item_url = $root_dir . '/mp3info.php?type=discuss&info=' . rawurlencode($genre) . '/' . rawurlencode($retArray[$lctr]) . '&cur_theme=' . $_SESSION['cur_theme'];
        ...
        switch ($directory_level) {
            case '3': # 3 directories deep
                if ('' == $genre) {
                    $item_url = $this_page . $url_seperator . 'ptype=genre&genre=' . rawurlencode($retArray[$lctr]);
                } else {
                    $item_url = $this_page . $url_seperator . 'ptype=artist&genre=' . rawurlencode($genre) . '&artist=' . rawurlencode($retArray[$lctr]);
                }
                break;
            case '2': # 2 directories deep
                if ('' == $genre) {
                    $item_url = $this_page . $url_seperator . 'ptype=artist&genre=' . urlencode($retArray[$lctr]);
                } else {
                    $item_url = $this_page . $url_seperator . 'ptype=songs&genre=&artist=&album=' . urlencode($retArray[$lctr]);
                }
                break;
        ...
        if ('true' == $enable_ratings) {
            ...
            echo displayRating(jzstripslashes($genre . '/' . $retArray[$lctr]));
        }

        // Now let's return the description

        $descData = returnDescription($retArray[$lctr]);
