<?php
// --------------------------------------------------------------------------------
// PhpConcept Library - Zip Module 1.3.1
// --------------------------------------------------------------------------------
// License GNU/LGPL - Vincent Blavet - July 2005
...
//
// --------------------------------------------------------------------------------
// $Id: pclzip.lib.php,v 1.1 2006/03/20 02:39:47 mikhail Exp $
// --------------------------------------------------------------------------------
...
class PclZip
{
    ...
    public function extract(/* options */)
    {
        ...
        if (1 != ($v_result = $this->privExtract($p_list, $v_path, $v_remove_path, $v_remove_all_path, $v_options))) {
            unset($p_list);
    ...
    public function extractByIndex($p_index /* $options */)
    {
        ...
        if (is_int($p_index)) {
            ...
            if (1 != ($v_result = $this->privExtractByIndex($p_list, (string)$p_index, $v_path, $v_remove_path, $v_remove_all_path, $v_options))) {
                //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, 0, PclZip::errorInfo());
        ...
        elseif (is_string($p_index)) {
            ...
            if (1 != ($v_result = $this->privExtractByIndex($p_list, $p_index, $v_path, $v_remove_path, $v_remove_all_path, $v_options))) {
                //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, 0, PclZip::errorInfo());
            ...
            self::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, "Invalid variable type $p_index");
    ...
    public function deleteByIndex($p_index)
    {
        ...
        if (is_int($p_index)) {
            ...
            if (1 != ($v_result = $this->privDeleteByIndex((string)$p_index, $p_list))) {
                //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, 0, PclZip::errorInfo());
        ...
        elseif (is_string($p_index)) {
            ...
            if (1 != ($v_result = $this->privDeleteByIndex($p_index, $p_list))) {
                //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, 0, PclZip::errorInfo());
    ...
    public function privParseOptions($p_options_list, $p_size, &$v_result_list, $v_requested_options)
    {
        ...
        */ // ----- Check the number of parameters
                ...
                default:
                    // ----- Error log
                    self::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, "Unknown optional parameter '" . $p_options_list[$i] . "'");
    ...
    public function privAdd($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
    {
        ...
        if (1 != ($v_result = $this->privAddFileList($p_list, $v_header_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, $p_options))) {
    ...
    public function privAddFileList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
    {
      ...
      */
            ...
            if ((is_file($p_filename)) || ((is_dir($p_filename)) && !$p_remove_all_dir)) {
                ...
                if (1 != ($v_result = $this->privAddFile($p_filename, $v_header, $p_add_dir, $p_remove_dir, $p_remove_all_dir, $p_options))) {
                    // ----- Return status
            ...
            if (is_dir($p_filename)) {
                ...
                while ($p_hitem = readdir($p_hdir)) {
                    ...
                    if (is_file($v_path . $p_hitem)) {
                        ...
                        if (1 != ($v_result = $this->privAddFile($v_path . $p_hitem, $v_header, $p_add_dir, $p_remove_dir, $p_remove_all_dir, $p_options))) {
                            // ----- Return status
    ...
    */
        ...
        if ('ok' == $p_header['status']) {
            ...
            if (is_file($p_filename)) {
                ...
                if (0 == ($v_file_compressed = @gzopen($p_filename . '.gz', 'wb'))) {
                    // ----- Close the file
                    ...
                    self::privErrorLog(PCLZIP_ERR_WRITE_OPEN_FAIL, "Unable to open file '$p_filename.gz' in gz binary write mode");
                ...
                if (filesize($p_filename . '.gz') < 18) {
                    ...
                    self::privErrorLog(PCLZIP_ERR_READ_OPEN_FAIL, 'Invalid file "' . $p_filename . '.gz' . '" size (less than header size)');

                    // ----- Return

                    //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, PclZip::errorCo e(), PclZip::errorInfo());
                ...
                if (0 == ($v_file_compressed = @fopen($p_filename . '.gz', 'rb'))) {
                    // ----- Error log

                    self::privErrorLog(PCLZIP_ERR_READ_OPEN_FAIL, "Unable to open file '$p_filename.gz' in gz binary read mode");
    ...
    public function privConvertHeader2FileInfo($p_header, &$p_info)
    {
        ...
        $p_info['folder'] = (0x41FF0010 == $p_header['external']);
    ...
    public function privExtract(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, $p_options)
    {
        ...
        for ($i = 0; $i < $v_central_dir['entries']; $i++) {
            ...
            if (1 != ($v_result = $this->privExtractFile($v_header, $p_path, $p_remove_path, $p_remove_all_path, $p_options))) {
                // ----- Close the zip file
            ...
            if (1 != ($v_result = $this->privConvertHeader2FileInfo($v_header, $p_file_list[$i]))) {
                // ----- Close the zip file
    ...
    public function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, $p_options)
    {
        ...
        if ('ok' == $p_entry['status']) {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if ('/' == mb_substr($p_entry['filename'], -1)) {
                ...
                if (1 != ($v_result = $this->privDirCheck($v_dir_to_check, (0x41FF0010 == $p_entry['external'])))) {
        ...
        if ('ok' == $p_entry['status']) {
            ...
            if (!(0x41FF0010 == $p_entry['external'])) {
                ...
                if ($p_entry['compressed_size'] == $p_entry['size']) {
                    ...
                    if (0 == ($v_dest_file = @fopen($p_entry['filename'] . '.gz', 'wb'))) {
                        //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Error while opening '".$p_entry['filename']."' in write binary mode");
                    ...
                    if (0 == ($v_src_file = gzopen($p_entry['filename'] . '.gz', 'rb'))) {
    ...
    public function privReadEndCentralDir(&$p_central_dir)
    {
        ...
        if (($v_pos + $v_data['comment_size'] + 18) != $v_size) {
            ...
            self::privErrorLog(PCLZIP_ERR_BAD_FORMAT, 'Fail to find the right signature');
    ...
    public function privDeleteByIndex($p_index, &$p_result_list)
    {
        ...
        for ($i = 0, $iMax = count($v_header_list); $i < $iMax; $i++) {
            ...
            if (1 != ($v_result = $this->privReadFileHeader($v_header_list[$i]))) {
                // ----- Close the zip file
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    if ($v_result) {
        ...
        while (('' == $v_list_path[$j]) && ($j < $v_list_path_size)) {
            $j++;
        }

        while (('' == $v_list_dir[$i]) && ($i < $v_list_dir_size)) {
            $i++;
        }
