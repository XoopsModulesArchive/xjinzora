...
class getid3_monkey
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        $ThisFileInfo['monkeys_audio']['raw'] = [];
        ...
        $thisfile_monkeysaudio_raw['magic'] = mb_substr($MACheaderData, 0, 4);

        if ('MAC ' != $thisfile_monkeysaudio_raw['magic']) {
        ...
        $thisfile_monkeysaudio_raw['nVersion'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 4, 2));

        $thisfile_monkeysaudio_raw['nCompressionLevel'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 6, 2));

        $thisfile_monkeysaudio_raw['nFormatFlags'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 8, 2));

        $thisfile_monkeysaudio_raw['nChannels'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 10, 2));

        $thisfile_monkeysaudio_raw['nSampleRate'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 12, 4));

        $thisfile_monkeysaudio_raw['nWAVHeaderBytes'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 16, 4));

        $thisfile_monkeysaudio_raw['nWAVTerminatingBytes'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 20, 4));

        $thisfile_monkeysaudio_raw['nTotalFrames'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 24, 4));

        $thisfile_monkeysaudio_raw['nFinalFrameSamples'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 28, 4));

        $thisfile_monkeysaudio_raw['nPeakLevel'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 32, 4));

        $thisfile_monkeysaudio_raw['nSeekElements'] = getid3_lib::LittleEndian2Int(mb_substr($MACheaderData, 38, 2));
        ...
        if (0 == $thisfile_monkeysaudio['bits_per_sample']) {
        ...
        if (0 == $thisfile_monkeysaudio['sample_rate']) {
        ...
        if (0 == $thisfile_monkeysaudio['playtime']) {
        ...
        if (0 == $thisfile_monkeysaudio['uncompressed_size']) {
    ...
    public function MonkeyCompressionLevelNameLookup($compressionlevel)
    {
        ...
        return ($MonkeyCompressionLevelNameLookup[$compressionlevel] ?? 'invalid');
    }
    ...
    public function MonkeySamplesPerFrame($versionid, $compressionlevel)
    {
        if ($versionid >= 3950) {
            ...
            return 73728;
        } elseif (($versionid >= 3800) && (4000 == $compressionlevel)) {
            return 73728;
        }
