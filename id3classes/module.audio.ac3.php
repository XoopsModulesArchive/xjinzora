...
class getid3_ac3
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        $thisfile_ac3_raw['synchinfo']['synchword'] = mb_substr($AC3header['syncinfo'], 0, 2);

        if ("\x0B\x77" != $thisfile_ac3_raw['synchinfo']['synchword']) {
            $ThisFileInfo['error'][] = 'Expecting "\x0B\x77" at offset ' . $ThisFileInfo['avdataoffset'] . ', found \x' . mb_strtoupper(dechex($AC3header['syncinfo'][0])) . '\x' . mb_strtoupper(dechex($AC3header['syncinfo'][1])) . ' instead';
        ...
        $thisfile_ac3_raw['synchinfo']['crc1'] = getid3_lib::LittleEndian2Int(mb_substr($AC3header['syncinfo'], 2, 2));

        $ac3_synchinfo_fscod_frmsizecod = getid3_lib::LittleEndian2Int(mb_substr($AC3header['syncinfo'], 4, 1));
        ...
        $thisfile_ac3_raw_bsi['bsid'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 5));
        ...
        $thisfile_ac3_raw_bsi['bsmod'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 3));

        $ac3_bsi_offset += 3;

        $thisfile_ac3_raw_bsi['acmod'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 3));
        ...
        if ($thisfile_ac3_raw_bsi['acmod'] & 0x01) {
            ...
            $thisfile_ac3_raw_bsi['cmixlev'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 2));
        ...
        if ($thisfile_ac3_raw_bsi['acmod'] & 0x04) {
            ...
            $thisfile_ac3_raw_bsi['surmixlev'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 2));
        ...
        if (0x02 == $thisfile_ac3_raw_bsi['acmod']) {
            // When operating in the two channel mode, this 2-bit code indicates whether or not the program has been encoded in Dolby Surround.

            $thisfile_ac3_raw_bsi['dsurmod'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 2));
        ...
        $thisfile_ac3_raw_bsi['lfeon'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));
        ...
        $thisfile_ac3_raw_bsi['dialnorm'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 5));

        $ac3_bsi_offset += 5;

        $thisfile_ac3['dialogue_normalization'] = '-' . $thisfile_ac3_raw_bsi['dialnorm'] . 'dB';

        $thisfile_ac3_raw_bsi['compre_flag'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        if ($thisfile_ac3_raw_bsi['compre_flag']) {
            $thisfile_ac3_raw_bsi['compr'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 8));
        ...
        $thisfile_ac3_raw_bsi['langcode_flag'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        if ($thisfile_ac3_raw_bsi['langcode_flag']) {
            $thisfile_ac3_raw_bsi['langcod'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 8));

            $ac3_bsi_offset += 8;
        }

        $thisfile_ac3_raw_bsi['audprodie'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        if ($thisfile_ac3_raw_bsi['audprodie']) {
            $thisfile_ac3_raw_bsi['mixlevel'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 5));

            $ac3_bsi_offset += 5;

            $thisfile_ac3_raw_bsi['roomtyp'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 2));
        ...
        if (0x00 == $thisfile_ac3_raw_bsi['acmod']) {
            ...
            $thisfile_ac3_raw_bsi['dialnorm2'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 5));

            $ac3_bsi_offset += 5;

            $thisfile_ac3['dialogue_normalization2'] = '-' . $thisfile_ac3_raw_bsi['dialnorm2'] . 'dB';

            $thisfile_ac3_raw_bsi['compre_flag2'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

            $ac3_bsi_offset += 1;

            if ($thisfile_ac3_raw_bsi['compre_flag2']) {
                $thisfile_ac3_raw_bsi['compr2'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 8));
            ...
            $thisfile_ac3_raw_bsi['langcode_flag2'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

            $ac3_bsi_offset += 1;

            if ($thisfile_ac3_raw_bsi['langcode_flag2']) {
                $thisfile_ac3_raw_bsi['langcod2'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 8));

                $ac3_bsi_offset += 8;
            }

            $thisfile_ac3_raw_bsi['audprodie2'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

            $ac3_bsi_offset += 1;

            if ($thisfile_ac3_raw_bsi['audprodie2']) {
                $thisfile_ac3_raw_bsi['mixlevel2'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 5));

                $ac3_bsi_offset += 5;

                $thisfile_ac3_raw_bsi['roomtyp2'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 2));
        ...
        $thisfile_ac3_raw_bsi['copyright'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        $thisfile_ac3_raw_bsi['original'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        $thisfile_ac3_raw_bsi['timecode1_flag'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        if ($thisfile_ac3_raw_bsi['timecode1_flag']) {
            $thisfile_ac3_raw_bsi['timecode1'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 14));

            $ac3_bsi_offset += 14;
        }

        $thisfile_ac3_raw_bsi['timecode2_flag'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        if ($thisfile_ac3_raw_bsi['timecode2_flag']) {
            $thisfile_ac3_raw_bsi['timecode2'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 14));

            $ac3_bsi_offset += 14;
        }

        $thisfile_ac3_raw_bsi['addbsi_flag'] = (bool)bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 1));

        $ac3_bsi_offset += 1;

        if ($thisfile_ac3_raw_bsi['addbsi_flag']) {
            $thisfile_ac3_raw_bsi['addbsi_length'] = bindec(mb_substr($AC3header['bsi'], $ac3_bsi_offset, 6));

            $ac3_bsi_offset += 6;

            $AC3header['bsi'] .= getid3_lib::BigEndian2Bin(fread($fd, $thisfile_ac3_raw_bsi['addbsi_length']));

            $thisfile_ac3_raw_bsi['addbsi_data'] = mb_substr($AC3header['bsi'], $ac3_bsi_offset, $thisfile_ac3_raw_bsi['addbsi_length'] * 8);
    ...
    public function AC3sampleRateCodeLookup($fscod)
    {
        ...
        return ($AC3sampleRateCodeLookup[$fscod] ?? false);
    }

    public function AC3serviceTypeLookup($bsmod, $acmod)
    {
        ...
        return ($AC3serviceTypeLookup[$bsmod][$acmod] ?? false);
    }

    public function AC3audioCodingModeLookup($acmod)
    {
        ...
        return ($AC3audioCodingModeLookup[$acmod] ?? false);
    }

    public function AC3centerMixLevelLookup($cmixlev)
    {
        ...
        return ($AC3centerMixLevelLookup[$cmixlev] ?? false);
    }

    public function AC3surroundMixLevelLookup($surmixlev)
    {
        ...
        return ($AC3surroundMixLevelLookup[$surmixlev] ?? false);
    }

    public function AC3dolbySurroundModeLookup($dsurmod)
    {
        ...
        return ($AC3dolbySurroundModeLookup[$dsurmod] ?? false);
    }

    public function AC3channelsEnabledLookup($acmod, $lfeon)
    {
    ...
    public function AC3heavyCompression($compre)
    {
        ...
        if ('1' == $fourbit[0]) {
            $log_gain = -8 + bindec(mb_substr($fourbit, 1));
        } else {
            $log_gain = bindec(mb_substr($fourbit, 1));
        }
    ...
    public function AC3roomTypeLookup($roomtyp)
    {
        ...
        return ($AC3roomTypeLookup[$roomtyp] ?? false);
    }

    public function AC3frameSizeLookup($frmsizecod, $fscod)
    {
        ...
        return ($AC3frameSizeLookup[$framesizeid][$fscod] ?? false);
    }

    public function AC3bitrateLookup($frmsizecod)
    {
        $framesizeid = floor($frmsizecod / 2);
        ...
        return ($AC3bitrateLookup[$framesizeid] ?? false);
    }
}
