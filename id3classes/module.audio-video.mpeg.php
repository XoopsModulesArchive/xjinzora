...
class getid3_mpeg
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        $MPEGstreamDataLength = mb_strlen($MPEGstreamData);

        $foundVideo = true;

        $VideoChunkOffset = 0;

        while (GETID3_MPEG_VIDEO_SEQUENCE_HEADER !== mb_substr($MPEGstreamData, $VideoChunkOffset++, 4)) {
        ...
        if ($foundVideo) {
            ...
            $VideoChunkOffset += (mb_strlen(GETID3_MPEG_VIDEO_SEQUENCE_HEADER) - 1);

            $FrameSizeDWORD = getid3_lib::BigEndian2Int(mb_substr($MPEGstreamData, $VideoChunkOffset, 3));

            $VideoChunkOffset += 3;

            $AspectRatioFrameRateDWORD = getid3_lib::BigEndian2Int(mb_substr($MPEGstreamData, $VideoChunkOffset, 1));

            $VideoChunkOffset += 1;

            $assortedinformation = getid3_lib::BigEndian2Bin(mb_substr($MPEGstreamData, $VideoChunkOffset, 4));
            ...
            $ThisFileInfo['mpeg']['video']['raw']['bitrate'] = getid3_lib::Bin2Dec(mb_substr($assortedinformation, 0, 18));

            $ThisFileInfo['mpeg']['video']['raw']['marker_bit'] = (bool)getid3_lib::Bin2Dec(mb_substr($assortedinformation, 18, 1));

            $ThisFileInfo['mpeg']['video']['raw']['vbv_buffer_size'] = getid3_lib::Bin2Dec(mb_substr($assortedinformation, 19, 10));

            $ThisFileInfo['mpeg']['video']['raw']['constrained_param_flag'] = (bool)getid3_lib::Bin2Dec(mb_substr($assortedinformation, 29, 1));

            $ThisFileInfo['mpeg']['video']['raw']['intra_quant_flag'] = (bool)getid3_lib::Bin2Dec(mb_substr($assortedinformation, 30, 1));

            if ($ThisFileInfo['mpeg']['video']['raw']['intra_quant_flag']) {
                // read 512 bits

                $ThisFileInfo['mpeg']['video']['raw']['intra_quant'] = getid3_lib::BigEndian2Bin(mb_substr($MPEGstreamData, $VideoChunkOffset, 64));

                $VideoChunkOffset += 64;

                $ThisFileInfo['mpeg']['video']['raw']['non_intra_quant_flag'] = (bool)getid3_lib::Bin2Dec(mb_substr($ThisFileInfo['mpeg']['video']['raw']['intra_quant'], 511, 1));

                $ThisFileInfo['mpeg']['video']['raw']['intra_quant'] = getid3_lib::Bin2Dec(mb_substr($assortedinformation, 31, 1)) . mb_substr(getid3_lib::BigEndian2Bin(mb_substr($MPEGstreamData, $VideoChunkOffset, 64)), 0, 511);

                if ($ThisFileInfo['mpeg']['video']['raw']['non_intra_quant_flag']) {
                    $ThisFileInfo['mpeg']['video']['raw']['non_intra_quant'] = mb_substr($MPEGstreamData, $VideoChunkOffset, 64);

                    $VideoChunkOffset += 64;
                }
            } else {
                $ThisFileInfo['mpeg']['video']['raw']['non_intra_quant_flag'] = (bool)getid3_lib::Bin2Dec(mb_substr($assortedinformation, 31, 1));
            ...
            if (0x3FFFF == $ThisFileInfo['mpeg']['video']['raw']['bitrate']) { // 18 set bits
        ...
        if (GETID3_MPEG_VIDEO_EXTENSION_START == mb_substr($MPEGstreamData, $VideoChunkOffset, 4)) {
        ...
        while (true) {
            while (GETID3_MPEG_AUDIO_START !== mb_substr($MPEGstreamData, $AudioChunkOffset++, 4)) {
    ...
    public function MPEGvideoAspectRatioTextLookup($rawaspectratio)
    {
        ...
        return ($MPEGvideoAspectRatioTextLookup[$rawaspectratio] ?? '');
    }
}
