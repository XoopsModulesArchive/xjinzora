...
class getid3_shorten
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        if ('ajkg' != mb_substr($ShortenHeader, 0, 4)) {
            $ThisFileInfo['error'][] = 'Expecting "ajkg" at offset ' . $ThisFileInfo['avdataoffset'] . ', found "' . mb_substr($ShortenHeader, 0, 4) . '"';
        ...
        $ThisFileInfo['shn']['version'] = getid3_lib::LittleEndian2Int(mb_substr($ShortenHeader, 4, 1));

        fseek($fd, $ThisFileInfo['avdataend'] - 12, SEEK_SET);

        $SeekTableSignatureTest = fread($fd, 12);

        $ThisFileInfo['shn']['seektable']['present'] = (bool)('SHNAMPSK' == mb_substr($SeekTableSignatureTest, 4, 8));

        if ($ThisFileInfo['shn']['seektable']['present']) {
            $ThisFileInfo['shn']['seektable']['length'] = getid3_lib::LittleEndian2Int(mb_substr($SeekTableSignatureTest, 0, 4));
            ...
            $ThisFileInfo['shn']['seektable']['entry_count'] = floor(mb_strlen($SeekTableData) / 80);
        ...
        if (!empty($output) && ('fmt ' == mb_substr($output, 12, 4))) {
            getid3_lib::IncludeDependency(GETID3_INCLUDEPATH . 'module.audio-video.riff.php', __FILE__, true);

            $DecodedWAVFORMATEX = getid3_riff::RIFFparseWAVEFORMATex(mb_substr($output, 20, 16));
            ...
            if ('data' == mb_substr($output, 36, 4)) {
                $ThisFileInfo['playtime_seconds'] = getid3_lib::LittleEndian2Int(mb_substr($output, 40, 4)) / $DecodedWAVFORMATEX['raw']['nAvgBytesPerSec'];
            } else {
