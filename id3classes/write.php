...
class getid3_writetags
{
    ...
    public $tagformats = [];        // array of tag formats to write ('id3v1', 'id3v2.2', 'id2v2.3', 'id3v2.4', 'ape', 'vorbiscomment', 'metaflac')
    ...
    public function WriteTags()
    {
        ...
        foreach ($this->tagformats as $requested_tag_format) {
            if (!in_array($requested_tag_format, $AllowedTagFormats, true)) {
        ...
        if ($this->remove_other_tags) {
            foreach ($AllowedTagFormats as $AllowedTagFormat) {
                switch ($AllowedTagFormat) {
                    ...
                    case 'id3v2.3':
                    case 'id3v2.4':
                        if (!in_array('id3v2', $TagFormatsToRemove, true) && !in_array('id3v2.2', $this->tagformats, true) && !in_array('id3v2.3', $this->tagformats, true) && !in_array('id3v2.4', $this->tagformats, true)) {
                            $TagFormatsToRemove[] = 'id3v2';
                        }
                        break;
                    default:
                        if (!in_array($AllowedTagFormat, $this->tagformats, true)) {
        ...
        foreach ($this->tag_data as $tag_key => $tag_array) {
            if (mb_strtoupper($tag_key) !== $tag_key) {
                $this->tag_data[mb_strtoupper($tag_key)] = $this->tag_data[$tag_key];
        ...
        if (!empty($this->ThisFileInfo['tags'])) {
            foreach ($this->ThisFileInfo['tags'] as $tag_format => $tag_data_array) {
                foreach ($tag_data_array as $tag_key => $tag_array) {
                    if (mb_strtoupper($tag_key) !== $tag_key) {
                        $this->ThisFileInfo['tags'][$tag_format][mb_strtoupper($tag_key)] = $this->ThisFileInfo['tags'][$tag_format][$tag_key];
        ...
        foreach ($this->tagformats as $tagformat) {
            ...
            switch ($tagformat) {
                case 'ape':
                    ...
                    if (false !== ($ape_writer->tag_data = $this->FormatDataForAPE())) {
                        $ape_writer->filename = $this->filename;

                        if (false === ($success = $ape_writer->WriteAPEtag())) {
                ...
                case 'id3v1':
                    ...
                    if (false !== ($id3v1_writer->tag_data = $this->FormatDataForID3v1())) {
                        $id3v1_writer->filename = $this->filename;

                        if (false === ($success = $id3v1_writer->WriteID3v1())) {
                ...
                case 'id3v2.4':
                    ...
                    $id3v2_writer->majorversion = (int)mb_substr($tagformat, -1);
                    $id3v2_writer->paddedlength = $this->id3v2_paddedlength;
                    if (false !== ($id3v2_writer->tag_data = $this->FormatDataForID3v2($id3v2_writer->majorversion))) {
                        $id3v2_writer->filename = $this->filename;

                        if (false === ($success = $id3v2_writer->WriteID3v2())) {
                ...
                case 'vorbiscomment':
                    ...
                    if (false !== ($vorbiscomment_writer->tag_data = $this->FormatDataForVorbisComment())) {
                        $vorbiscomment_writer->filename = $this->filename;

                        if (false === ($success = $vorbiscomment_writer->WriteVorbisComment())) {
                ...
                case 'metaflac':
                    ...
                    if (false !== ($metaflac_writer->tag_data = $this->FormatDataForMetaFLAC())) {
                        $metaflac_writer->filename = $this->filename;

                        if (false === ($success = $metaflac_writer->WriteMetaFLAC())) {
        ...
        foreach ($TagFormatsToDelete as $DeleteTagFormat) {
            ...
            switch ($DeleteTagFormat) {
                case 'id3v1':
                    $id3v1_writer = new getid3_write_id3v1();
                    $id3v1_writer->filename = $this->filename;
                    if (false === ($success = $id3v1_writer->RemoveID3v1())) {
                ...
                case 'id3v2':
                    $id3v2_writer = new getid3_write_id3v2();
                    $id3v2_writer->filename = $this->filename;
                    if (false === ($success = $id3v2_writer->RemoveID3v2())) {
                ...
                case 'ape':
                    $ape_writer = new getid3_write_apetag();
                    $ape_writer->filename = $this->filename;
                    if (false === ($success = $ape_writer->DeleteAPEtag())) {
                ...
                case 'vorbiscomment':
                    $vorbiscomment_writer = new getid3_write_vorbiscomment();
                    $vorbiscomment_writer->filename = $this->filename;
                    if (false === ($success = $vorbiscomment_writer->DeleteVorbisComment())) {
                ...
                case 'metaflac':
                    $metaflac_writer = new getid3_write_metaflac();
                    $metaflac_writer->filename = $this->filename;
                    if (false === ($success = $metaflac_writer->DeleteMetaFLAC())) {
                ...
                case 'lyrics3':
                    ...
                    if (false === ($success = $lyrics3_writer->DeleteLyrics3())) {
    ...
    public function FormatDataForID3v1()
    {
        ...
        $tag_data_id3v1['track'] = (int)getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', @implode(' ', @$this->tag_data['TRACKNUMBER']));
            ...
            foreach ($valuearray as $key => $value) {
                ...
                if (mb_strstr($value, "\n")) {
                    ...
                    foreach ($multilineexploded as $newcomment) {
                        if (mb_strlen(trim($newcomment)) > 0) {
