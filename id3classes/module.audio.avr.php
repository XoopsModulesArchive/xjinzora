...
class getid3_avr
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        $ThisFileInfo['avr']['raw']['magic'] = mb_substr($AVRheader, 0, 4);

        if ('2BIT' != $ThisFileInfo['avr']['raw']['magic']) {
        ...
        $ThisFileInfo['avr']['sample_name'] = rtrim(mb_substr($AVRheader, 4, 8));

        $ThisFileInfo['avr']['raw']['mono'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 12, 2));

        $ThisFileInfo['avr']['bits_per_sample'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 14, 2));

        $ThisFileInfo['avr']['raw']['signed'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 16, 2));

        $ThisFileInfo['avr']['raw']['loop'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 18, 2));

        $ThisFileInfo['avr']['raw']['midi'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 20, 2));

        $ThisFileInfo['avr']['raw']['replay_freq'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 22, 1));

        $ThisFileInfo['avr']['sample_rate'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 23, 3));

        $ThisFileInfo['avr']['sample_length'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 26, 4));

        $ThisFileInfo['avr']['loop_start'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 30, 4));

        $ThisFileInfo['avr']['loop_end'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 34, 4));

        $ThisFileInfo['avr']['midi_split'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 38, 2));

        $ThisFileInfo['avr']['sample_compression'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 40, 2));

        $ThisFileInfo['avr']['reserved'] = getid3_lib::BigEndian2Int(mb_substr($AVRheader, 42, 2));

        $ThisFileInfo['avr']['sample_name_extra'] = rtrim(mb_substr($AVRheader, 44, 20));

        $ThisFileInfo['avr']['comment'] = rtrim(mb_substr($AVRheader, 64, 64));

        $ThisFileInfo['avr']['flags']['stereo'] = ((0 == $ThisFileInfo['avr']['raw']['mono']) ? false : true);

        $ThisFileInfo['avr']['flags']['signed'] = ((0 == $ThisFileInfo['avr']['raw']['signed']) ? false : true);

        $ThisFileInfo['avr']['flags']['loop'] = ((0 == $ThisFileInfo['avr']['raw']['loop']) ? false : true);

        $ThisFileInfo['avr']['midi_notes'] = [];

        if (0xFF00 != ($ThisFileInfo['avr']['raw']['midi'] & 0xFF00)) {
            $ThisFileInfo['avr']['midi_notes'][] = ($ThisFileInfo['avr']['raw']['midi'] & 0xFF00) >> 8;
        }

        if (0x00FF != ($ThisFileInfo['avr']['raw']['midi'] & 0x00FF)) {
            $ThisFileInfo['avr']['midi_notes'][] = ($ThisFileInfo['avr']['raw']['midi'] & 0x00FF);
        }

        if (($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']) != ($ThisFileInfo['avr']['sample_length'] * ((8 == $ThisFileInfo['avr']['bits_per_sample']) ? 1 : 2))) {
            $ThisFileInfo['warning'][] = 'Probable truncated file: expecting ' . ($ThisFileInfo['avr']['sample_length'] * ((8 == $ThisFileInfo['avr']['bits_per_sample']) ? 1 : 2)) . ' bytes of audio data, found ' . ($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']);
        }
        ...
        $ThisFileInfo['audio']['bitrate'] = ($ThisFileInfo['avr']['sample_length'] * ((8 == $ThisFileInfo['avr']['bits_per_sample']) ? 8 : 16)) / $ThisFileInfo['playtime_seconds'];
