...
class getid3_apetag
{
    public function __construct($fd, &$ThisFileInfo, $overrideendoffset = 0)
    {
        $id3v1tagsize = 128;
        ...
        if (0 == $overrideendoffset) {
            ...
            if ('APETAGEX' == mb_substr($APEfooterID3v1, mb_strlen($APEfooterID3v1) - $id3v1tagsize - $apetagheadersize, 8)) {
                // APE tag found before ID3v1

                $ThisFileInfo['ape']['tag_offset_end'] = $ThisFileInfo['filesize'] - $id3v1tagsize;

            //} elseif (preg_match('/APETAGEX.{24}$/i', $APEfooterID3v1)) {
            } elseif ('APETAGEX' == mb_substr($APEfooterID3v1, mb_strlen($APEfooterID3v1) - $apetagheadersize, 8)) {
        ...
        if (isset($thisfile_ape['footer']['flags']['header']) && $thisfile_ape['footer']['flags']['header']) {
            if ($thisfile_ape['header'] = $this->parseAPEheaderFooter(mb_substr($APEtagData, 0, $apetagheadersize))) {
        ...
        $ThisFileInfo['replay_gain'] = [];

        $thisfile_replaygain = &$ThisFileInfo['replay_gain'];

        for ($i = 0; $i < $thisfile_ape['footer']['raw']['tag_items']; $i++) {
            $value_size = getid3_lib::LittleEndian2Int(mb_substr($APEtagData, $offset, 4));

            $offset += 4;

            $item_flags = getid3_lib::LittleEndian2Int(mb_substr($APEtagData, $offset, 4));

            $offset += 4;

            if (false === mb_strstr(mb_substr($APEtagData, $offset), "\x00")) {
            ...
            $ItemKeyLength = mb_strpos($APEtagData, "\x00", $offset) - $offset;

            $item_key = mb_strtolower(mb_substr($APEtagData, $offset, $ItemKeyLength));
            ...
            $thisfile_ape_items_current['data'] = mb_substr($APEtagData, $offset, $value_size);
            ...
            switch (mb_strtolower($item_key)) {
                ...
                case 'mp3gain_undo':
                    ...
                    $thisfile_replaygain['mp3gain']['undo_wrap'] = (('Y' == $mp3gain_undo_wrap) ? true : false);
                    break;
                case 'mp3gain_minmax':
                ...
                default:
                    foreach ($thisfile_ape_items_current['data'] as $comment) {
                        $thisfile_ape['comments'][mb_strtolower($item_key)][] = $comment;
                    }
                    break;
            }
        }
    ...
    public function parseAPEheaderFooter($APEheaderFooterData)
    {
        ...
        $headerfooterinfo_raw['footer_tag'] = mb_substr($APEheaderFooterData, 0, 8);

        if ('APETAGEX' != $headerfooterinfo_raw['footer_tag']) {
            return false;
        }

        $headerfooterinfo_raw['version'] = getid3_lib::LittleEndian2Int(mb_substr($APEheaderFooterData, 8, 4));

        $headerfooterinfo_raw['tagsize'] = getid3_lib::LittleEndian2Int(mb_substr($APEheaderFooterData, 12, 4));

        $headerfooterinfo_raw['tag_items'] = getid3_lib::LittleEndian2Int(mb_substr($APEheaderFooterData, 16, 4));

        $headerfooterinfo_raw['global_flags'] = getid3_lib::LittleEndian2Int(mb_substr($APEheaderFooterData, 20, 4));

        $headerfooterinfo_raw['reserved'] = mb_substr($APEheaderFooterData, 24, 8);
    ...
    public function APEtagItemIsUTF8Lookup($itemkey)
    {
        ...
        return in_array(mb_strtolower($itemkey), $APEtagItemIsUTF8Lookup, true);
    }
}
