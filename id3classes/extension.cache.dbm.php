 ...
 *
 *       require_once __DIR__ . '/getid3/getid3.php';
 *       require_once __DIR__ . '/getid3/getid3/extension.cache.dbm.php';
...
class getID3_cached_dbm extends getID3
{
    ...
    public function __construct($cache_type, $dbm_filename, $lock_filename)
    {
        ...
        if (function_exists('dbaHandlers')) {  // PHP 4.3.0+
            if (!in_array('db3', dbaHandlers(), true)) {
            ...
            if (!mb_strstr($contents, $cache_type)) {
                die('PHP is not compiled --with ' . $cache_type . ' support, required to use DBM style cache.');
            }
        }
        ...
        $this->lock = fopen($lock_filename, 'wb');
        ...
        if (GETID3_VERSION != dba_fetch(GETID3_VERSION, $this->dba)) {
            $this->clear_cache();
        }
