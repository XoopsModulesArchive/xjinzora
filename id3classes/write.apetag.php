...
class getid3_write_apetag
{
    ...
    public function WriteAPEtag()
    {
        ...
        if ($this->always_preserve_replaygain) {
            ...
            foreach ($ReplayGainTagsToPreserve as $rg_key) {
                if (isset($ThisFileInfo['ape']['items'][mb_strtolower($rg_key)]['data'][0]) && !isset($this->tag_data[mb_strtoupper($rg_key)][0])) {
                    $this->tag_data[mb_strtoupper($rg_key)][0] = $ThisFileInfo['ape']['items'][mb_strtolower($rg_key)]['data'][0];
                }
            }
        }
        ...
        if ($APEtag = $this->GenerateAPEtag()) {
            if ($fp = @fopen($this->filename, 'a+b')) {
                ...
                fwrite($fp, $APEtag, mb_strlen($APEtag));

                fwrite($fp, $PostAPEdata, mb_strlen($PostAPEdata));
    ...
    public function DeleteAPEtag()
    {
        ...
        if (isset($ThisFileInfo['ape']['tag_offset_start']) && isset($ThisFileInfo['ape']['tag_offset_end'])) {
            if ($fp = @fopen($this->filename, 'a+b')) {
                ...
                fwrite($fp, $DataAfterAPE, mb_strlen($DataAfterAPE));
    ...
    public function GenerateAPEtag()
    {
        ...
        foreach ($this->tag_data as $key => $arrayofvalues) {
            ...
            $tagitem = getid3_lib::LittleEndian2String(mb_strlen($valuestring), 4);
    ...
    public function GenerateAPEtagHeaderFooter($items, $isheader = false)
    {
        ...
        foreach ($items as $itemdata) {
            $tagdatalength += mb_strlen($itemdata);
        }
    ...
    public function CleanAPEtagItemKey($itemkey)
    {
        ...
        switch (mb_strtoupper($itemkey)) {
            case 'EAN/UPC':
            case 'ISBN':
            case 'LC':
            case 'ISRC':
                $itemkey = mb_strtoupper($itemkey);
                break;
            default:
