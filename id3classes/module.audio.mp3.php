...
class getid3_mp3
{
    ...
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        if (((isset($ThisFileInfo['id3v2']['headerlength']) && ($ThisFileInfo['avdataoffset'] > $ThisFileInfo['id3v2']['headerlength'])) || (!isset($ThisFileInfo['id3v2']) && ($ThisFileInfo['avdataoffset'] > 0)))) {
            ...
            if ('cbr' == $ThisFileInfo['audio']['bitrate_mode']) {
        ...
        if (!empty($CurrentDataLAMEversionString) && ('LAME3.' == mb_substr($CurrentDataLAMEversionString, 0, 6)) && !preg_match('[0-9\)]', mb_substr($CurrentDataLAMEversionString, -1))) {
            ...
            switch (mb_substr($CurrentDataLAMEversionString, -1)) {
                case 'a':
                case 'b':
                    // "LAME3.94a" will have a longer version string of "LAME3.94 (alpha)" for example
                    // need to trim off "a" to match longer string
                    $CurrentDataLAMEversionString = mb_substr($CurrentDataLAMEversionString, 0, -1);
                    break;
            }

            if (false !== ($PossiblyLongerLAMEversion_String = mb_strstr($PossiblyLongerLAMEversion_Data, $CurrentDataLAMEversionString))) {
                if (mb_substr($PossiblyLongerLAMEversion_String, 0, mb_strlen($CurrentDataLAMEversionString)) == $CurrentDataLAMEversionString) {
                    $PossiblyLongerLAMEversion_NewString = mb_substr($PossiblyLongerLAMEversion_String, 0, strspn($PossiblyLongerLAMEversion_String, 'LAME0123456789., (abcdefghijklmnopqrstuvwxyzJFSOND)')); //"LAME3.90.3"  "LAME3.87 (beta 1, Sep 27 2000)" "LAME3.88 (beta)"

                    if (mb_strlen($PossiblyLongerLAMEversion_NewString) > mb_strlen(@$ThisFileInfo['audio']['encoder'])) {
        ...
        if ('mp3' == $ThisFileInfo['fileformat']) {
    ...
    public function GuessEncoderOptions(&$ThisFileInfo)
    {
        // shortcuts
        ...
        if (('Fraunhofer' == @$thisfile_mpeg_audio['VBR_method']) && !empty($thisfile_mpeg_audio['VBR_quality'])) {
        ...
        } elseif (!empty($thisfile_mpeg_audio_lame['preset_used']) && (!in_array($thisfile_mpeg_audio_lame['preset_used_id'], $NamedPresetBitrates, true))) {
            ...
            } elseif ('vbr' == $ThisFileInfo['audio']['bitrate_mode']) {
            ...
            } elseif ('cbr' == $ThisFileInfo['audio']['bitrate_mode']) {
                $encoder_options = mb_strtoupper($ThisFileInfo['audio']['bitrate_mode']) . ceil($ThisFileInfo['audio']['bitrate'] / 1000);
            } else {
                $encoder_options = mb_strtoupper($ThisFileInfo['audio']['bitrate_mode']);
            }
    ...
    public function decodeMPEGaudioHeader($fd, $offset, &$ThisFileInfo, $recursivesearch = true, $ScanAsCBR = false, $FastMPEGheaderScan = false)
    {
        ...
        $head4 = mb_substr($headerstring, 0, 4);
        ...
        if (!$FastMPEGheaderScan) {
            ...
            if ($thisfile_mpeg_audio['protection']) {
                $thisfile_mpeg_audio['crc'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, 4, 2));
            }
        }
        ...
        if (('free' == $thisfile_mpeg_audio['bitrate']) && ($offset == $ThisFileInfo['avdataoffset'])) {
        ...
        if (!$FastMPEGheaderScan && ('2' == $thisfile_mpeg_audio['layer'])) {
        ...
        if ($ThisFileInfo['audio']['sample_rate'] > 0) {
            $thisfile_mpeg_audio['framelength'] = self::MPEGaudioFrameLength($thisfile_mpeg_audio['bitrate'], $thisfile_mpeg_audio['version'], $thisfile_mpeg_audio['layer'], (int)$thisfile_mpeg_audio['padding'], $ThisFileInfo['audio']['sample_rate']);
        }
        ...
        if ('VBRI' == mb_substr($headerstring, 4 + 32, 4)) {
            ...
            $SideInfoData = mb_substr($headerstring, 4 + 2, 32);

            $FraunhoferVBROffset = 36;

            $thisfile_mpeg_audio['VBR_encoder_version'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 4, 2)); // VbriVersion
            $thisfile_mpeg_audio['VBR_encoder_delay'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 6, 2)); // VbriDelay
            $thisfile_mpeg_audio['VBR_quality'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 8, 2)); // VbriQuality
            $thisfile_mpeg_audio['VBR_bytes'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 10, 4)); // VbriStreamBytes
            $thisfile_mpeg_audio['VBR_frames'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 14, 4)); // VbriStreamFrames
            $thisfile_mpeg_audio['VBR_seek_offsets'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 18, 2)); // VbriTableSize
            $thisfile_mpeg_audio['VBR_seek_scale'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 20, 2)); // VbriTableScale
            $thisfile_mpeg_audio['VBR_entry_bytes'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 22, 2)); // VbriEntryBytes
            $thisfile_mpeg_audio['VBR_entry_frames'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset + 24, 2)); // VbriEntryFrames
            ...
            for ($i = 0; $i < $thisfile_mpeg_audio['VBR_seek_offsets']; $i++) {
                $Fraunhofer_OffsetN = getid3_lib::BigEndian2Int(mb_substr($headerstring, $FraunhoferVBROffset, $thisfile_mpeg_audio['VBR_entry_bytes']));
            ...
            $SideInfoData = mb_substr($headerstring, 4 + 2, $VBRidOffset - 4);

            if (('Xing' == mb_substr($headerstring, $VBRidOffset, mb_strlen('Xing'))) || ('Info' == mb_substr($headerstring, $VBRidOffset, mb_strlen('Info')))) {
                ...
                $thisfile_mpeg_audio['xing_flags_raw'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $VBRidOffset + 4, 4));
                ...
                if ($thisfile_mpeg_audio['xing_flags']['frames']) {
                    $thisfile_mpeg_audio['VBR_frames'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $VBRidOffset + 8, 4));

                    //$thisfile_mpeg_audio['VBR_frames']--; // don't count header Xing/Info frame
                }

                if ($thisfile_mpeg_audio['xing_flags']['bytes']) {
                    $thisfile_mpeg_audio['VBR_bytes'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $VBRidOffset + 12, 4));
                }
                ...
                if ($thisfile_mpeg_audio['xing_flags']['toc']) {
                    $LAMEtocData = mb_substr($headerstring, $VBRidOffset + 16, 100);
                ...
                if ($thisfile_mpeg_audio['xing_flags']['vbr_scale']) {
                    $thisfile_mpeg_audio['VBR_scale'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $VBRidOffset + 116, 4));
                }

                // http://gabriel.mp3-tech.org/mp3infotag.html

                if ('LAME' == mb_substr($headerstring, $VBRidOffset + 120, 4)) {
                    // shortcut
                    ...
                    $thisfile_mpeg_audio_lame['long_version'] = mb_substr($headerstring, $VBRidOffset + 120, 20);

                    $thisfile_mpeg_audio_lame['short_version'] = mb_substr($thisfile_mpeg_audio_lame['long_version'], 0, 9);
                    ...
                    if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.90') {
                        ...
                        $thisfile_mpeg_audio_lame['vbr_quality'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0x9B, 1));

                        // bytes $9C-$A4  Encoder short VersionString

                        $thisfile_mpeg_audio_lame['short_version'] = mb_substr($headerstring, $LAMEtagOffsetContant + 0x9C, 9);

                        // byte $A5  Info Tag revision + VBR method

                        $LAMEtagRevisionVBRmethod = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xA5, 1));
                        ...
                        $thisfile_mpeg_audio['bitrate_mode'] = mb_substr($thisfile_mpeg_audio_lame['vbr_method'], 0, 3); // usually either 'cbr' or 'vbr', but truncates 'vbr-old / vbr-rh' to 'vbr'

                        // byte $A6  Lowpass filter value

                        $thisfile_mpeg_audio_lame['lowpass_frequency'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xA6, 1)) * 100;
                        ...
                        if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.94b') {
                            ...
                            $thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] = (float)((getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xA7, 4))) / 8388608);
                        } else {
                            // LAME 3.94a15 and earlier - 32-bit floating point

                            // Actually 3.94a16 will fall in here too and be WRONG, but is hard to detect 3.94a16 vs 3.94a15

                            $thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] = getid3_lib::LittleEndian2Float(mb_substr($headerstring, $LAMEtagOffsetContant + 0xA7, 4));
                        }
                        ...
                        $thisfile_mpeg_audio_lame_raw['RGAD_track'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xAB, 2));

                        $thisfile_mpeg_audio_lame_raw['RGAD_album'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xAD, 2));
                        ...
                        $EncodingFlagsATHtype = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xAF, 1));
                        ...
                        $thisfile_mpeg_audio_lame['raw']['abrbitrate_minbitrate'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xB0, 1));
                        ...
                        $EncoderDelays = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xB1, 3));
                        ...
                        $MiscByte = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xB4, 1));
                        ...
                        $thisfile_mpeg_audio_lame_raw['mp3_gain'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xB5, 1), false, true);
                        ...
                        $PresetSurroundBytes = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xB6, 2));
                        ...
                        $thisfile_mpeg_audio_lame['audio_bytes'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xB8, 4));
                        ...
                        $thisfile_mpeg_audio_lame['music_crc'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xBC, 2));

                        // bytes $BE-$BF  CRC-16 of Info Tag

                        $thisfile_mpeg_audio_lame['lame_tag_crc'] = getid3_lib::BigEndian2Int(mb_substr($headerstring, $LAMEtagOffsetContant + 0xBE, 2));
                ...
                if ($recursivesearch) {
                    ...
                    if (self::RecursiveFrameScanning($fd, $ThisFileInfo, $offset, $nextframetestoffset, true)) {
                        $recursivesearch = false;
        ...
        if (($ExpectedNumberOfAudioBytes > 0) && (($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']) != $ExpectedNumberOfAudioBytes)) {
            if ($ExpectedNumberOfAudioBytes > ($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset'])) {
                if (1 == ($ExpectedNumberOfAudioBytes - ($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']))) {
                    $ThisFileInfo['warning'][] = 'Last byte of data truncated (this is a known bug in Meracl ID3 Tag Writer before v1.3.5)';
                } else {
                    $ThisFileInfo['warning'][] = 'Probable truncated file: expecting '
                                                 . $ExpectedNumberOfAudioBytes
                                                 . ' bytes of audio data, only found '
                                                 . ($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset'])
                                                 . ' (short by '
                                                 . ($ExpectedNumberOfAudioBytes - ($ThisFileInfo['avdataend']
                                                                                   - $ThisFileInfo['avdataoffset']))
                                                 . ' bytes)';
                }
            } else {
                if (1 == (($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']) - $ExpectedNumberOfAudioBytes)) {
                ...
                //		$ThisFileInfo['warning'][] = 'Extra null byte at end of MP3 data assumed to be RIFF padding and therefore ignored';
                    ...
                    $ThisFileInfo['warning'][] = 'Too much data in file: expecting ' . $ExpectedNumberOfAudioBytes . ' bytes of audio data, found ' . ($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']) . ' (' . (($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset'])
                                                                                                                                                                                                                             - $ExpectedNumberOfAudioBytes) . ' bytes too many)';
                }
            }
        }

        if (('free' == $thisfile_mpeg_audio['bitrate']) && empty($ThisFileInfo['audio']['bitrate'])) {
            if (($offset == $ThisFileInfo['avdataoffset']) && empty($thisfile_mpeg_audio['VBR_frames'])) {
                $framebytelength = self::FreeFormatFrameLength($fd, $offset, $ThisFileInfo, true);
                ...
                if ($framebytelength > 0) {
                    ...
                    if ('1' == $thisfile_mpeg_audio['layer']) {
                        ...
                        $ThisFileInfo['audio']['bitrate'] = ((($framebytelength / 4) - (int)$thisfile_mpeg_audio['padding']) * $thisfile_mpeg_audio['sample_rate']) / 12;
                    } else {
                        // Bitrate = ((FrameLengthInBytes - Padding) * SampleRate) / 144

                        $ThisFileInfo['audio']['bitrate'] = (($framebytelength - (int)$thisfile_mpeg_audio['padding']) * $thisfile_mpeg_audio['sample_rate']) / 144;
                    }
                } else {
        ...
        if ($recursivesearch) {
            if (!self::RecursiveFrameScanning($fd, $ThisFileInfo, $offset, $nextframetestoffset, $ScanAsCBR)) {
                return false;
            }
        }
    ...
    public function RecursiveFrameScanning($fd, &$ThisFileInfo, $offset, &$nextframetestoffset, $ScanAsCBR)
    {
        for ($i = 0; $i < GETID3_MP3_VALID_CHECK_FRAMES; $i++) {
            ...
            if (self::decodeMPEGaudioHeader($fd, $nextframetestoffset, $nextframetestarray, false)) {
                if ($ScanAsCBR) {
    ...
    public function FreeFormatFrameLength($fd, $offset, &$ThisFileInfo, $deepscan = false)
    {
        fseek($fd, $offset, SEEK_SET);

        $MPEGaudioData = fread($fd, 32768);

        $SyncPattern1 = mb_substr($MPEGaudioData, 0, 4);
        ...
        $framelength1 = mb_strpos($MPEGaudioData, $SyncPattern1, 4);

        $framelength2 = mb_strpos($MPEGaudioData, $SyncPattern2, 4);
        ...
        if (!$framelength) {
            ...
            $framelength1 = mb_strpos($MPEGaudioData, mb_substr($SyncPattern1, 0, 3), 4);

            $framelength2 = mb_strpos($MPEGaudioData, mb_substr($SyncPattern2, 0, 3), 4);
            ...
            $SyncPattern1 = mb_substr($SyncPattern1, 0, 3);

            $SyncPattern2 = mb_substr($SyncPattern2, 0, 3);
        }
        ...
        if ($deepscan) {
            ...
            while ($nextoffset < ($ThisFileInfo['avdataend'] - 6)) {
                ...
                if ((mb_substr($NextSyncPattern, 1, mb_strlen($SyncPattern1)) == $SyncPattern1) || (mb_substr($NextSyncPattern, 1, mb_strlen($SyncPattern2)) == $SyncPattern2)) {
                    // good - found where expected

                    $ActualFrameLengthValues[] = $framelength;
                } elseif ((mb_substr($NextSyncPattern, 0, mb_strlen($SyncPattern1)) == $SyncPattern1) || (mb_substr($NextSyncPattern, 0, mb_strlen($SyncPattern2)) == $SyncPattern2)) {
                    // ok - found one byte earlier than expected (last frame wasn't padded, first frame was)

                    $ActualFrameLengthValues[] = ($framelength - 1);

                    $nextoffset--;
                } elseif ((mb_substr($NextSyncPattern, 2, mb_strlen($SyncPattern1)) == $SyncPattern1) || (mb_substr($NextSyncPattern, 2, mb_strlen($SyncPattern2)) == $SyncPattern2)) {
    ...
    public function getOnlyMPEGaudioInfoBruteForce($fd, &$ThisFileInfo)
    {
                ...
                if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
                    ...
                    if ("\xFF" == $next4[0]) {
                        ...
                        if ($MPEGaudioHeaderValidCache[$next4]) {
                            ...
                            @$Distribution['padding'][(int)$LongMPEGpaddingLookup[$head4]]++;
        ...
        if (0 == $ThisFileInfo['mpeg']['audio']['frame_count']) {
    ...
    public function getOnlyMPEGaudioInfo($fd, &$ThisFileInfo, $avdataoffset, $BitrateHistogram = false)
    {
        ...
        $header_len = mb_strlen($header) - round(GETID3_FREAD_BUFFER_SIZE / 2);
        ...
        while (true) {
            if (($SynchSeekOffset > $header_len) && (($avdataoffset + $SynchSeekOffset) < $ThisFileInfo['avdataend']) && !feof($fd)) {
                if ($SynchSeekOffset > 131072) {
                    ...
                    if (isset($ThisFileInfo['mpeg']) && (!is_array($ThisFileInfo['mpeg']) || (0 == count($ThisFileInfo['mpeg'])))) {
                        unset($ThisFileInfo['mpeg']);
                    }
            ...
            if (($SynchSeekOffset + 1) >= mb_strlen($header)) {
                $ThisFileInfo['error'][] = 'could not find valid MPEG synch before end of file';

                return false;
            }

            if (("\xFF" == $header[$SynchSeekOffset]) && ($header[($SynchSeekOffset + 1)] > "\xE0")) { // synch detected
                if (!isset($FirstFrameThisfileInfo) && !isset($ThisFileInfo['mpeg']['audio'])) {
                    $FirstFrameThisfileInfo = $ThisFileInfo;

                    $FirstFrameAVDataOffset = $avdataoffset + $SynchSeekOffset;

                    if (!self::decodeMPEGaudioHeader($fd, $avdataoffset + $SynchSeekOffset, $FirstFrameThisfileInfo, false)) {
                ...
                if (self::decodeMPEGaudioHeader($fd, $avdataoffset + $SynchSeekOffset, $dummy, true)) {
                    $ThisFileInfo = $dummy;
                    ...
                    if (isset($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode']) && ('vbr' == $FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode'])) {
                        if (!(abs($ThisFileInfo['audio']['bitrate'] - $FirstFrameThisfileInfo['audio']['bitrate']) <= 1)) {
                            ...
                            if (self::decodeMPEGaudioHeader($fd, $GarbageOffsetEnd, $dummy, true, true)) {
                                $ThisFileInfo = $dummy;

                                $ThisFileInfo['avdataoffset'] = $GarbageOffsetEnd;

                                $ThisFileInfo['warning'][] = 'apparently-valid VBR header not used because could not find '
                                ...
                                $ThisFileInfo['warning'][] = 'using data from VBR header even though could not find '
                                                             . GETID3_MP3_VALID_CHECK_FRAMES
                                                             . ' consecutive MPEG-audio frames immediately after VBR header (garbage data for '
                                                             . ($GarbageOffsetEnd - $GarbageOffsetStart)
                                                             . ' bytes between '
                    ...
                    if (isset($ThisFileInfo['mpeg']['audio']['bitrate_mode']) && ('vbr' == $ThisFileInfo['mpeg']['audio']['bitrate_mode']) && !isset($ThisFileInfo['mpeg']['audio']['VBR_method'])) {
                    ...
                    if ($BitrateHistogram) {
                        ...
                        if ('1' == $ThisFileInfo['mpeg']['audio']['version']) {
                            if (3 == $ThisFileInfo['mpeg']['audio']['layer']) {
                                $ThisFileInfo['mpeg']['audio']['bitrate_distribution'] = ['free' => 0, 32000 => 0, 40000 => 0, 48000 => 0, 56000 => 0, 64000 => 0, 80000 => 0, 96000 => 0, 112000 => 0, 128000 => 0, 160000 => 0, 192000 => 0, 224000 => 0, 256000 => 0, 320000 => 0];
                            } elseif (2 == $ThisFileInfo['mpeg']['audio']['layer']) {
                                $ThisFileInfo['mpeg']['audio']['bitrate_distribution'] = ['free' => 0, 32000 => 0, 48000 => 0, 56000 => 0, 64000 => 0, 80000 => 0, 96000 => 0, 112000 => 0, 128000 => 0, 160000 => 0, 192000 => 0, 224000 => 0, 256000 => 0, 320000 => 0, 384000 => 0];
                            } elseif (1 == $ThisFileInfo['mpeg']['audio']['layer']) {
                                $ThisFileInfo['mpeg']['audio']['bitrate_distribution'] = ['free' => 0, 32000 => 0, 64000 => 0, 96000 => 0, 128000 => 0, 160000 => 0, 192000 => 0, 224000 => 0, 256000 => 0, 288000 => 0, 320000 => 0, 352000 => 0, 384000 => 0, 416000 => 0, 448000 => 0];
                            }
                        } elseif (1 == $ThisFileInfo['mpeg']['audio']['layer']) {
                            $ThisFileInfo['mpeg']['audio']['bitrate_distribution'] = ['free' => 0, 32000 => 0, 48000 => 0, 56000 => 0, 64000 => 0, 80000 => 0, 96000 => 0, 112000 => 0, 128000 => 0, 144000 => 0, 160000 => 0, 176000 => 0, 192000 => 0, 224000 => 0, 256000 => 0];
                        } else {
                            $ThisFileInfo['mpeg']['audio']['bitrate_distribution'] = ['free' => 0, 8000 => 0, 16000 => 0, 24000 => 0, 32000 => 0, 40000 => 0, 48000 => 0, 56000 => 0, 64000 => 0, 80000 => 0, 96000 => 0, 112000 => 0, 128000 => 0, 144000 => 0, 160000 => 0];
                        }
                        ...
                        while (self::decodeMPEGaudioHeader($fd, $synchstartoffset, $dummy, false, false, $FastMode)) {
                            $FastMode = true;
    ...
    public function MPEGaudioVersionArray()
    {
    ...
    public function MPEGaudioLayerArray()
    {
    ...
    public function MPEGaudioBitrateArray()
    {
    ...
    public function MPEGaudioFrequencyArray()
    {
    ...
    public function MPEGaudioChannelModeArray()
    {
    ...
    public function MPEGaudioModeExtensionArray()
    {
    ...
    public function MPEGaudioEmphasisArray()
    {
    ...
    public function MPEGaudioHeaderBytesValid($head4, $allowBitrate15 = false)
    {
        return self::MPEGaudioHeaderValid(self::MPEGaudioHeaderDecode($head4), false, $allowBitrate15);
    }

    public function MPEGaudioHeaderValid($rawarray, $echoerrors = false, $allowBitrate15 = false)
    {
    ...
    public function MPEGaudioHeaderDecode($Header4Bytes)
    {
        ...
        if (4 != mb_strlen($Header4Bytes)) {
            return false;
        }

        $MPEGrawHeader['synch'] = (getid3_lib::BigEndian2Int(mb_substr($Header4Bytes, 0, 2)) & 0xFFE0) >> 4;
    ...
    public function MPEGaudioFrameLength($bitrate, $version, $layer, $padding, $samplerate)
    {
    ...
    public function ClosestStandardMP3Bitrate($bitrate)
    {
        static $StandardBitrates = [320000, 256000, 224000, 192000, 160000, 128000, 112000, 96000, 80000, 64000, 56000, 48000, 40000, 32000, 24000, 16000, 8000];

        static $BitrateTable = [0 => '-'];
    ...
    public function XingVBRidOffset($version, $channelmode)
    {
    ...
    public function LAMEvbrMethodLookup($VBRmethodID)
    {
    ...
    public function LAMEmiscStereoModeLookup($StereoModeID)
    {
    ...
    public function LAMEmiscSourceSampleFrequencyLookup($SourceSampleFrequencyID)
    {
    ...
    public function LAMEsurroundInfoLookup($SurroundInfoID)
    {
    ...
    public function LAMEpresetUsedLookup($LAMEtag)
    {
        if (0 == $LAMEtag['preset_used_id']) {
        ...
        if (empty($LAMEpresetUsedLookup)) {
            ...
            if ($LAMEtag['short_version'] >= 'LAME3.94') {
                ...
                $LAMEpresetUsedLookup[320] = '--preset insane';                                                             // 3.94a15 Nov 12 2005
                $LAMEpresetUsedLookup[430] = '--preset radio';                                                              // 3.94a15 Nov 12 2005
                $LAMEpresetUsedLookup[450] = '--preset ' . ((4 == $LAMEtag['raw']['vbr_method']) ? 'fast ' : '') . 'portable'; // 3.94a15 Nov 12 2005
                $LAMEpresetUsedLookup[460] = '--preset ' . ((4 == $LAMEtag['raw']['vbr_method']) ? 'fast ' : '') . 'medium';   // 3.94a15 Nov 12 2005
                $LAMEpresetUsedLookup[470] = '--r3mix';                                                                     // 3.94b1  Dec 18 2005
                $LAMEpresetUsedLookup[480] = '--preset ' . ((4 == $LAMEtag['raw']['vbr_method']) ? 'fast ' : '') . 'standard'; // 3.94a15 Nov 12 2005
                $LAMEpresetUsedLookup[500] = '--preset ' . ((4 == $LAMEtag['raw']['vbr_method']) ? 'fast ' : '') . 'extreme';  // 3.94a15 Nov 12 2005
                $LAMEpresetUsedLookup[1010] = '--preset portable';                                                           // 3.94a15 Oct 21 2005
                $LAMEpresetUsedLookup[1015] = '--preset radio';                                                              // 3.94a15 Oct 21 2005
            }
        }
