...
class getid3_bonk
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        while ($this->BonkIsValidTagName(mb_substr($PossibleBonkTag, 4, 4), true)) {
            $BonkTagSize = getid3_lib::LittleEndian2Int(mb_substr($PossibleBonkTag, 0, 4));
            ...
            if (("\x00" != $TagHeaderTest[0]) || (mb_substr($PossibleBonkTag, 4, 4) != mb_strtolower(mb_substr($PossibleBonkTag, 4, 4)))) {
                $ThisFileInfo['error'][] = 'Expecting "Ø' . mb_strtoupper(mb_substr($PossibleBonkTag, 4, 4)) . '" at offset ' . $BonkTagOffset . ', found "' . $TagHeaderTest . '"';

                return false;
            }

            $BonkTagName = mb_substr($TagHeaderTest, 1, 4);
    ...
    public function HandleBonkTags($fd, $BonkTagName, &$ThisFileInfo)
    {
        switch ($BonkTagName) {
            case 'BONK':
                // shortcut
                ...
                $thisfile_bonk_BONK['version'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 5, 1));
                $thisfile_bonk_BONK['number_samples'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 6, 4));
                $thisfile_bonk_BONK['sample_rate'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 10, 4));

                $thisfile_bonk_BONK['channels'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 14, 1));
                $thisfile_bonk_BONK['lossless'] = (bool)getid3_lib::LittleEndian2Int(mb_substr($BonkData, 15, 1));
                $thisfile_bonk_BONK['joint_stereo'] = (bool)getid3_lib::LittleEndian2Int(mb_substr($BonkData, 16, 1));
                $thisfile_bonk_BONK['number_taps'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 17, 2));
                $thisfile_bonk_BONK['downsampling_ratio'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 19, 1));
                $thisfile_bonk_BONK['samples_per_packet'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 20, 2));
            ...
            case 'INFO':
                ...
                if (!$this->BonkIsValidTagName(mb_substr($NextInfoDataPair, 1, 4))) {
                    while (!feof($fd)) {
            ...
            case 'META':
                ...
                $ThisFileInfo['bonk']['META']['version'] = getid3_lib::LittleEndian2Int(mb_substr($BonkData, 5, 1));

                $MetaTagEntries = floor(((mb_strlen($BonkData) - 8) - 6) / 8); // BonkData - xxxxmeta - ØMETA
                $offset = 6;
                for ($i = 0; $i < $MetaTagEntries; $i++) {
                    $MetaEntryTagName = mb_substr($BonkData, $offset, 4);

                    $offset += 4;

                    $MetaEntryTagOffset = getid3_lib::LittleEndian2Int(mb_substr($BonkData, $offset, 4));
    ...
    public function BonkIsValidTagName($PossibleBonkTag, $ignorecase = false)
    {
        ...
        foreach ($BonkIsValidTagName as $validtagname) {
            if ($validtagname == $PossibleBonkTag) {
                return true;
            } elseif ($ignorecase && (mb_strtolower($validtagname) == mb_strtolower($PossibleBonkTag))) {
                return true;
            }
        }
