...
class getid3_rkau
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        if ('RKA' != mb_substr($RKAUHeader, 0, 3)) {
            $ThisFileInfo['error'][] = 'Expecting "RKA" at offset ' . $ThisFileInfo['avdataoffset'] . ', found "' . mb_substr($RKAUHeader, 0, 3) . '"';
        ...
        $ThisFileInfo['rkau']['raw']['version'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 3, 1));
        ...
        $ThisFileInfo['rkau']['source_bytes'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 4, 4));

        $ThisFileInfo['rkau']['sample_rate'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 8, 4));

        $ThisFileInfo['rkau']['channels'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 12, 1));

        $ThisFileInfo['rkau']['bits_per_sample'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 13, 1));

        $ThisFileInfo['rkau']['raw']['quality'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 14, 1));

        $this->RKAUqualityLookup($ThisFileInfo['rkau']);

        $ThisFileInfo['rkau']['raw']['flags'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 15, 1));
        ...
        if ($ThisFileInfo['rkau']['flags']['streaming']) {
            ...
            $ThisFileInfo['rkau']['compressed_bytes'] = getid3_lib::LittleEndian2Int(mb_substr($RKAUHeader, 16, 4));
        } else {
    ...
    public function RKAUqualityLookup(&$RKAUdata)
    {
        ...
        $RKAUdata['lossless'] = ((0 == $quality) ? true : false);
