...
class getid3_la
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        switch (mb_substr($rawdata, $offset, 4)) {
            case 'LA02':
            case 'LA03':
            case 'LA04':
                ...
                $ThisFileInfo['la']['version_major'] = (int)mb_substr($rawdata, $offset + 2, 1);
                $ThisFileInfo['la']['version_minor'] = (int)mb_substr($rawdata, $offset + 3, 1);
                $ThisFileInfo['la']['version'] = (float)$ThisFileInfo['la']['version_major'] + ($ThisFileInfo['la']['version_minor'] / 10);
                $offset += 4;

                $ThisFileInfo['la']['uncompressed_size'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                $offset += 4;
                if (0 == $ThisFileInfo['la']['uncompressed_size']) {
                    $ThisFileInfo['error'][] = 'Corrupt LA file: uncompressed_size == zero';

                    return false;
                }

                $WAVEchunk = mb_substr($rawdata, $offset, 4);
                if ('WAVE' !== $WAVEchunk) {
                ...
                if ($ThisFileInfo['la']['version'] >= 0.3) {
                    $ThisFileInfo['la']['fmt_size'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                ...
                $fmt_chunk = mb_substr($rawdata, $offset, 4);
                if ('fmt ' !== $fmt_chunk) {
                ...
                $offset += 4;
                $fmt_size = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                $offset += 4;

                $ThisFileInfo['la']['raw']['format'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 2));
                $offset += 2;

                $ThisFileInfo['la']['channels'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 2));
                $offset += 2;
                if (0 == $ThisFileInfo['la']['channels']) {
                    $ThisFileInfo['error'][] = 'Corrupt LA file: channels == zero';

                    return false;
                }

                $ThisFileInfo['la']['sample_rate'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                $offset += 4;
                if (0 == $ThisFileInfo['la']['sample_rate']) {
                    $ThisFileInfo['error'][] = 'Corrupt LA file: sample_rate == zero';

                    return false;
                }

                $ThisFileInfo['la']['bytes_per_second'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                $offset += 4;
                $ThisFileInfo['la']['bytes_per_sample'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 2));
                $offset += 2;
                $ThisFileInfo['la']['bits_per_sample'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 2));
                $offset += 2;

                $ThisFileInfo['la']['samples'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                $offset += 4;

                $ThisFileInfo['la']['raw']['flags'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 1));
                $offset += 1;
                ...
                $ThisFileInfo['la']['original_crc'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                $offset += 4;
                ...
                if ($ThisFileInfo['la']['flags']['seekable']) {
                    ...
                    for ($i = 0; $i < $ThisFileInfo['la']['seekpoint_count']; $i++) {
                        $ThisFileInfo['la']['seekpoints'][] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                ...
                if ($ThisFileInfo['la']['version'] >= 0.3) {
                    ...
                    $ThisFileInfo['la']['footerstart'] = getid3_lib::LittleEndian2Int(mb_substr($rawdata, $offset, 4));
                ...
                if ($ThisFileInfo['la']['footerstart'] < $ThisFileInfo['avdataend']) {
                    if ($RIFFtempfilename = tempnam('*', 'id3')) {
                        if ($RIFF_fp = fopen($RIFFtempfilename, 'w+b')) {
                            ...
                            if (0.2 == $ThisFileInfo['la']['version']) {
                                $RIFFdata .= mb_substr($rawdata, 12, 24);
                            } else {
                                $RIFFdata .= mb_substr($rawdata, 16, 24);
                            }
                            ...
                            $RIFFdata = 'RIFF' . getid3_lib::LittleEndian2String(mb_strlen($RIFFdata), 4, false) . $RIFFdata;

                            fwrite($RIFF_fp, $RIFFdata, mb_strlen($RIFFdata));

                            $dummy = $ThisFileInfo;

                            $dummy['filesize'] = mb_strlen($RIFFdata);
            ...
            $ThisFileInfo['avdataoffset']  += $offset;
                ...
                break;
            default:
                if ('LA' == mb_substr($rawdata, $offset, 2)) {
                    $ThisFileInfo['error'][] = 'This version of getID3() (v' . GETID3_VERSION . ') doesn\'t support LA version ' . mb_substr($rawdata, $offset + 2, 1) . '.' . mb_substr($rawdata, $offset + 3, 1) . ' which this appears to be - check http://getid3.sourceforge.net for updates.';
                } else {
