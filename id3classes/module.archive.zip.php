...
class getid3_zip
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        while ($EOCDsearchCounter++ < 512) {
            ...
            if (mb_strstr($EOCDsearchData, 'PK' . "\x05\x06")) {
                $EOCDposition = mb_strpos($EOCDsearchData, 'PK' . "\x05\x06");
                ...
                if (0 == $ThisFileInfo['zip']['entries_count']) {
                ...
                if (isset($ThisFileInfo['zip']['compression_method']) && ('store' == $ThisFileInfo['zip']['compression_method']) && !isset($ThisFileInfo['zip']['compression_speed'])) {
    ...
    public function getZIPHeaderFilepointerTopDown($fd, &$ThisFileInfo)
    {
        $ThisFileInfo['fileformat'] = 'zip';
    ...
    public function getZIPentriesFilepointer($fd, &$ThisFileInfo)
    {
        $ThisFileInfo['zip']['compressed_size'] = 0;
    ...
    public function ZIPparseLocalFileHeader($fd)
    {
        $LocalFileHeader['offset'] = ftell($fd);

        $ZIPlocalFileHeader = fread($fd, 30);

        $LocalFileHeader['raw']['signature'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 0, 4));

        if (0x04034B50 != $LocalFileHeader['raw']['signature']) {
        ...
        $LocalFileHeader['raw']['extract_version'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 4, 2));

        $LocalFileHeader['raw']['general_flags'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 6, 2));

        $LocalFileHeader['raw']['compression_method'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 8, 2));

        $LocalFileHeader['raw']['last_mod_file_time'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 10, 2));

        $LocalFileHeader['raw']['last_mod_file_date'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 12, 2));

        $LocalFileHeader['raw']['crc_32'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 14, 4));

        $LocalFileHeader['raw']['compressed_size'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 18, 4));

        $LocalFileHeader['raw']['uncompressed_size'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 22, 4));

        $LocalFileHeader['raw']['filename_length'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 26, 2));

        $LocalFileHeader['raw']['extra_field_length'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPlocalFileHeader, 28, 2));
        ...
        if ($FilenameExtrafieldLength > 0) {
            ...
            if ($LocalFileHeader['raw']['filename_length'] > 0) {
                $LocalFileHeader['filename'] = mb_substr($ZIPlocalFileHeader, 30, $LocalFileHeader['raw']['filename_length']);
            }

            if ($LocalFileHeader['raw']['extra_field_length'] > 0) {
                $LocalFileHeader['raw']['extra_field_data'] = mb_substr($ZIPlocalFileHeader, 30 + $LocalFileHeader['raw']['filename_length'], $LocalFileHeader['raw']['extra_field_length']);
            }
        }
        ...
        if ($LocalFileHeader['flags']['data_descriptor_used']) {
            ...
            $LocalFileHeader['data_descriptor']['crc_32'] = getid3_lib::LittleEndian2Int(mb_substr($DataDescriptor, 0, 4));

            $LocalFileHeader['data_descriptor']['compressed_size'] = getid3_lib::LittleEndian2Int(mb_substr($DataDescriptor, 4, 4));

            $LocalFileHeader['data_descriptor']['uncompressed_size'] = getid3_lib::LittleEndian2Int(mb_substr($DataDescriptor, 8, 4));
        }

        return $LocalFileHeader;
    }

    public function ZIPparseCentralDirectory($fd)
    {
        $CentralDirectory['offset'] = ftell($fd);

        $ZIPcentralDirectory = fread($fd, 46);

        $CentralDirectory['raw']['signature'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 0, 4));

        if (0x02014B50 != $CentralDirectory['raw']['signature']) {
        ...
        $CentralDirectory['raw']['create_version'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 4, 2));

        $CentralDirectory['raw']['extract_version'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 6, 2));

        $CentralDirectory['raw']['general_flags'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 8, 2));

        $CentralDirectory['raw']['compression_method'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 10, 2));

        $CentralDirectory['raw']['last_mod_file_time'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 12, 2));

        $CentralDirectory['raw']['last_mod_file_date'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 14, 2));

        $CentralDirectory['raw']['crc_32'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 16, 4));

        $CentralDirectory['raw']['compressed_size'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 20, 4));

        $CentralDirectory['raw']['uncompressed_size'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 24, 4));

        $CentralDirectory['raw']['filename_length'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 28, 2));

        $CentralDirectory['raw']['extra_field_length'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 30, 2));

        $CentralDirectory['raw']['file_comment_length'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 32, 2));

        $CentralDirectory['raw']['disk_number_start'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 34, 2));

        $CentralDirectory['raw']['internal_file_attrib'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 36, 2));

        $CentralDirectory['raw']['external_file_attrib'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 38, 4));

        $CentralDirectory['raw']['local_header_offset'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPcentralDirectory, 42, 4));
        ...
        if ($FilenameExtrafieldCommentLength > 0) {
            ...
            if ($CentralDirectory['raw']['filename_length'] > 0) {
                $CentralDirectory['filename'] = mb_substr($FilenameExtrafieldComment, 0, $CentralDirectory['raw']['filename_length']);
            }

            if ($CentralDirectory['raw']['extra_field_length'] > 0) {
                $CentralDirectory['raw']['extra_field_data'] = mb_substr($FilenameExtrafieldComment, $CentralDirectory['raw']['filename_length'], $CentralDirectory['raw']['extra_field_length']);
            }

            if ($CentralDirectory['raw']['file_comment_length'] > 0) {
                $CentralDirectory['file_comment'] = mb_substr($FilenameExtrafieldComment, $CentralDirectory['raw']['filename_length'] + $CentralDirectory['raw']['extra_field_length'], $CentralDirectory['raw']['file_comment_length']);
            }
        }
    ...
    public function ZIPparseEndOfCentralDirectory($fd)
    {
        $EndOfCentralDirectory['offset'] = ftell($fd);

        $ZIPendOfCentralDirectory = fread($fd, 22);

        $EndOfCentralDirectory['signature'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 0, 4));

        if (0x06054B50 != $EndOfCentralDirectory['signature']) {
        ...
        $EndOfCentralDirectory['disk_number_current'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 4, 2));

        $EndOfCentralDirectory['disk_number_start_directory'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 6, 2));

        $EndOfCentralDirectory['directory_entries_this_disk'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 8, 2));

        $EndOfCentralDirectory['directory_entries_total'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 10, 2));

        $EndOfCentralDirectory['directory_size'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 12, 4));

        $EndOfCentralDirectory['directory_offset'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 16, 4));

        $EndOfCentralDirectory['comment_length'] = getid3_lib::LittleEndian2Int(mb_substr($ZIPendOfCentralDirectory, 20, 2));
    ...
    public function ZIPparseGeneralPurposeFlags($flagbytes, $compressionmethod)
    {
    ...
    public function ZIPversionOSLookup($index)
    {
    ...
    public function ZIPcompressionMethodLookup($index)
    {
    ...
    public function DOStime2UNIXtime($DOSdate, $DOStime)
    {
        // wFatDate
