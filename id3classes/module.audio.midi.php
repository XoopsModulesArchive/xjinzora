...
class getid3_midi
{
    public function __construct($fd, &$ThisFileInfo, $scanwholefile = true)
    {
        // shortcut

        $ThisFileInfo['midi']['raw'] = [];
        ...
        $MIDIheaderID = mb_substr($MIDIdata, $offset, 4); // 'MThd'

        if ('MThd' != $MIDIheaderID) {
        ...
        $thisfile_midi_raw['headersize'] = getid3_lib::BigEndian2Int(mb_substr($MIDIdata, $offset, 4));

        $offset += 4;

        $thisfile_midi_raw['fileformat'] = getid3_lib::BigEndian2Int(mb_substr($MIDIdata, $offset, 2));

        $offset += 2;

        $thisfile_midi_raw['tracks'] = getid3_lib::BigEndian2Int(mb_substr($MIDIdata, $offset, 2));

        $offset += 2;

        $thisfile_midi_raw['ticksperqnote'] = getid3_lib::BigEndian2Int(mb_substr($MIDIdata, $offset, 2));

        $offset += 2;

        for ($i = 0; $i < $thisfile_midi_raw['tracks']; $i++) {
            if ((mb_strlen($MIDIdata) - $offset) < 8) {
                $MIDIdata .= fread($fd, GETID3_FREAD_BUFFER_SIZE);
            }

            $trackID = mb_substr($MIDIdata, $offset, 4);

            $offset += 4;

            if ('MTrk' == $trackID) {
                $tracksize = getid3_lib::BigEndian2Int(mb_substr($MIDIdata, $offset, 4));

                $offset += 4;

                // $thisfile_midi['tracks'][$i]['size'] = $tracksize;

                $trackdataarray[$i] = mb_substr($MIDIdata, $offset, $tracksize);
        ...
        if ($scanwholefile) { // this can take quite a long time, so have the option to bypass it if speed is very important
            ...
            foreach ($trackdataarray as $tracknumber => $trackdata) {
                ...
                while ($eventsoffset < mb_strlen($trackdata)) {
                    $eventid = 0;
                    ...
                    for ($i = 0; $i < 4; $i++) {
                        $deltatimebyte = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    ...
                    $MIDI_event_channel = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    ...
                    if (0x8 == $MIDIevents[$tracknumber][$eventid]['eventid']) { // Note off (key is released)
                        $notenumber = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $velocity = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    } elseif (0x9 == $MIDIevents[$tracknumber][$eventid]['eventid']) { // Note on (key is pressed)
                        $notenumber = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $velocity = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    } elseif (0xA == $MIDIevents[$tracknumber][$eventid]['eventid']) { // Key after-touch
                        $notenumber = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $velocity = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    } elseif (0xB == $MIDIevents[$tracknumber][$eventid]['eventid']) { // Control Change
                        $controllernum = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $newvalue = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    } elseif (0xC == $MIDIevents[$tracknumber][$eventid]['eventid']) { // Program (patch) change
                        $newprogramnum = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    ...
                    } elseif (0xD == $MIDIevents[$tracknumber][$eventid]['eventid']) { // Channel after-touch
                        $channelnumber = ord(mb_substr($trackdata, $eventsoffset++, 1));
                    } elseif (0xE == $MIDIevents[$tracknumber][$eventid]['eventid']) { // Pitch wheel change (2000H is normal or no change)
                        $changeLSB = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $changeMSB = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $pitchwheelchange = (($changeMSB & 0x7F) << 7) & ($changeLSB & 0x7F);
                    } elseif ((0xF == $MIDIevents[$tracknumber][$eventid]['eventid']) && (0xF == $MIDIevents[$tracknumber][$eventid]['channel'])) {
                        $METAeventCommand = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $METAeventLength = ord(mb_substr($trackdata, $eventsoffset++, 1));

                        $METAeventData = mb_substr($trackdata, $eventsoffset, $METAeventLength);

                        $eventsoffset += $METAeventLength;

                        switch ($METAeventCommand) {
                            case 0x00: // Set track sequence number
                                $track_sequence_number = getid3_lib::BigEndian2Int(mb_substr($METAeventData, 0, $METAeventLength));
                                //$thisfile_midi_raw['events'][$tracknumber][$eventid]['seqno'] = $track_sequence_number;
                                break;
                            case 0x01: // Text: generic
                                $text_generic = mb_substr($METAeventData, 0, $METAeventLength);
                            ...
                            case 0x02: // Text: copyright
                                $text_copyright = mb_substr($METAeventData, 0, $METAeventLength);
                            ...
                            case 0x06: // Text: marker
                                $text_marker = mb_substr($METAeventData, 0, $METAeventLength);
                                //$thisfile_midi_raw['events'][$tracknumber][$eventid]['marker'] = $text_marker;
                                break;
                            case 0x07: // Text: cue point
                                $text_cuepoint = mb_substr($METAeventData, 0, $METAeventLength);
                            ...
                            case 0x51: // Tempo: microseconds / quarter note
                                $CurrentMicroSecondsPerBeat = getid3_lib::BigEndian2Int(mb_substr($METAeventData, 0, $METAeventLength));
                                if (0 == $CurrentMicroSecondsPerBeat) {
                            ...
                            case 0x7F: // Sequencer specific information
                                $custom_data = mb_substr($METAeventData, 0, $METAeventLength);
                                break;
                            default:
            ...
            foreach ($MicroSecondsPerQuarterNoteAfter as $tickoffset => $microsecondsperbeat) {
                if ($thisfile_midi['totalticks'] > $tickoffset) {
                    if (0 == $thisfile_midi_raw['ticksperqnote']) {
