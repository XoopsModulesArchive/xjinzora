...
class getid3_mpc
{
    public function __construct($fd, &$ThisFileInfo)
    {
        ...
        if ('MP+' == mb_substr($MPCheaderData, $offset, 3)) {
            // great, this is SV7+
            $thisfile_mpc_header['raw']['preamble'] = mb_substr($MPCheaderData, $offset, 3); // should be 'MP+'
            $offset += 3;
        } elseif (preg_match('/^[\x00\x01\x10\x11\x40\x41\x50\x51\x80\x81\x90\x91\xC0\xC1\xD0\xD1][\x20-37][\x00\x20\x40\x60\x80\xA0\xC0\xE0]/s', mb_substr($MPCheaderData, 0, 4))) {
            // this is SV4 - SV6, handle seperately

            // Most of this code adapted from Jurgen Faul's MPEGplus source code - thanks Jurgen! :)

            $HeaderDWORD[0] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, 0, 4));

            $HeaderDWORD[1] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, 4, 4));
            ...
            if (($thisfile_mpc_header['stream_major_version'] > 4) && (1 != $thisfile_mpc_header['block_size'])) {
            ...
            if (0 == $thisfile_mpc_header['target_bitrate']) {
                $ThisFileInfo['audio']['bitrate_mode'] = 'vbr';
            } else {
            ...
            $ThisFileInfo['error'][] = 'Expecting "MP+" at offset ' . $ThisFileInfo['avdataoffset'] . ', found "' . mb_substr($MPCheaderData, $offset, 3) . '"';
        ...
        $StreamVersionByte = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 1));
        ...
        $thisfile_mpc_header['frame_count'] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 4));
        ...
        $FlagsDWORD1 = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 4));
        ...
        $thisfile_mpc_header['raw']['title_peak'] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 2));

        $offset += 2;

        $thisfile_mpc_header['raw']['title_gain'] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 2), true);

        $offset += 2;

        $thisfile_mpc_header['raw']['album_peak'] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 2));

        $offset += 2;

        $thisfile_mpc_header['raw']['album_gain'] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 2), true);

        $offset += 2;

        $FlagsDWORD2 = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 4));
        ...
        $thisfile_mpc_header['raw']['not_sure_what'] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 3));

        $offset += 3;

        $thisfile_mpc_header['raw']['encoder_version'] = getid3_lib::LittleEndian2Int(mb_substr($MPCheaderData, $offset, 1));
        ...
        if (0 == $thisfile_mpc_header['sample_rate']) {
    ...
    public function MPCprofileNameLookup($profileid)
    {
        ...
        return ($MPCprofileNameLookup[$profileid] ?? 'invalid');
    }
    ...
    public function MPCfrequencyLookup($frequencyid)
    {
        ...
        return ($MPCfrequencyLookup[$frequencyid] ?? 'invalid');
    }
    ...
    public function MPCencoderVersionLookup($encoderversion)
    {
        ...
        if (0 == ($encoderversion % 10)) {
            // release version

            return number_format($encoderversion / 100, 2);
        } elseif (0 == ($encoderversion % 2)) {
            // beta version
