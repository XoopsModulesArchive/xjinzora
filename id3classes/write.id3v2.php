...
class getid3_write_id3v2
{
    ...
    public function WriteID3v2()
    {
        ...
        if (is_writable($this->filename) || (!file_exists($this->filename) && is_writable(dirname($this->filename)))) {
            ...
            if ($NewID3v2Tag = $this->GenerateID3v2Tag()) {
                if (file_exists($this->filename) && is_writable($this->filename) && isset($OldThisFileInfo['id3v2']['headerlength']) && ($OldThisFileInfo['id3v2']['headerlength'] == mb_strlen($NewID3v2Tag))) {
                    ...
                    if (file_exists($this->filename)) {
                        ...
                        if ($fp = fopen($this->filename, 'r+b')) {
                            ...
                            fwrite($fp, $NewID3v2Tag, mb_strlen($NewID3v2Tag));
                    ...
                    if ($tempfilename = tempnam('*', 'getID3')) {
                        ...
                        if ($fp_source = fopen($this->filename, 'rb')) {
                            if ($fp_temp = fopen($tempfilename, 'wb')) {
                                fwrite($fp_temp, $NewID3v2Tag, mb_strlen($NewID3v2Tag));
                                ...
                                while ($buffer = fread($fp_source, GETID3_FREAD_BUFFER_SIZE)) {
                                    fwrite($fp_temp, $buffer, mb_strlen($buffer));
                                }
    ...
    public function RemoveID3v2()
    {
        ...
        if (is_writable(dirname($this->filename))) {
            ...
            if ($fp_source = @fopen($this->filename, 'rb')) {
                ...
                if (false !== $OldThisFileInfo['avdataoffset']) {
                    fseek($fp_source, $OldThisFileInfo['avdataoffset'], SEEK_SET);
                }
                ...
                if ($fp_temp = tmpfile()) {
                    ...
                    if ($fp_source = @fopen($this->filename, 'wb')) {
                        ...
                        while ($buffer = fread($fp_temp, GETID3_FREAD_BUFFER_SIZE)) {
                            fwrite($fp_source, $buffer, mb_strlen($buffer));
                        }
        ...
        if (($this->majorversion < 3) || ($this->majorversion > 4)) {
            ...
            switch ($frame_name) {
                case 'UFID':
                    ...
                    if (mb_strlen($source_data_array['data']) > 64) {
                        $this->errors[] = 'Identifier not allowed to be longer than 64 bytes in ' . $frame_name . ' (supplied data was ' . mb_strlen($source_data_array['data']) . ' bytes long)';
                    } else {
                        $framedata .= str_replace("\x00", '', $source_data_array['ownerid']) . "\x00";

                        $framedata .= mb_substr($source_data_array['data'], 0, 64); // max 64 bytes - truncate anything longer
                    }
                    break;
                case 'TXXX':
                ...
                case 'ETCO':
                    ...
                    if (($source_data_array['timestampformat'] > 2) || ($source_data_array['timestampformat'] < 1)) {
                        ...
                        foreach ($source_data_array as $key => $val) {
                            ...
                            } elseif (('timestampformat' != $key) && ('flags' != $key)) {
                ...
                case 'MLLT':
                    ...
                    foreach ($source_data_array as $key => $val) {
                        if (('framesbetweenreferences' != $key) && ('bytesbetweenreferences' != $key) && ('msbetweenreferences' != $key) && ('bitsforbytesdeviation' != $key) && ('bitsformsdeviation' != $key) && ('flags' != $key)) {
                    ...
                    for ($i = 0, $iMax = mb_strlen($unwrittenbitstream); $i < $iMax; $i += 8) {
                        $highnibble = bindec(mb_substr($unwrittenbitstream, $i, 4)) << 4;

                        $lownibble = bindec(mb_substr($unwrittenbitstream, $i + 4, 4));
                ...
                case 'USLT':
                    ...
                    if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
                        ...
                        $framedata .= mb_strtolower($source_data_array['language']);
                ...
                case 'RVAD':
                    ...
                    if (!$this->IsWithinBitRange($source_data_array['bitsvolume'], 8, false)) {
                        ...
                        if ($source_data_array['volumechange']['rightrear']
                ...
                case 'LINK':
                    ...
                    if (!getid3_id3v2::IsValidID3v2FrameName($source_data_array['frameid'], $this->majorversion)) {
                        ...
                        $this->errors[] = 'Invalid URL in ' . $frame_name . ' (' . $source_data_array['data'] . ')';
                    } elseif ((('AENC' == $source_data_array['frameid']) || ('APIC' == $source_data_array['frameid']) || ('GEOB' == $source_data_array['frameid']) || ('TXXX' == $source_data_array['frameid'])) && ('' == $source_data_array['additionaldata'])) {
                    ...
                    } elseif (('USER' == $source_data_array['frameid']) && ('' == getid3_id3v2::LanguageLookup($source_data_array['additionaldata'], true))) {
                    ...
                    } elseif (('PRIV' == $source_data_array['frameid']) && ('' == $source_data_array['additionaldata'])) {
                    ...
                    } elseif ((('COMM' == $source_data_array['frameid']) || ('SYLT' == $source_data_array['frameid']) || ('USLT' == $source_data_array['frameid']))
                              && (('' == getid3_id3v2::LanguageLookup(mb_substr($source_data_array['additionaldata'], 0, 3), true))
                                  || ('' == mb_substr(
                        ...
                        switch ($source_data_array['frameid']) {
                            ...
                            case 'RBUF':
                                ...
                                // no break
                            default:
                                if (('T' == mb_substr($source_data_array['frameid'], 0, 1)) || ('W' == mb_substr($source_data_array['frameid'], 0, 1))) {
                                    // no additional data required
                                } else {
                                    $this->errors[] = $source_data_array['frameid'] . ' is not a valid Frame Identifier in ' . $frame_name . ' (in ID3v2.' . $this->majorversion . ')';
                                }
        ...
        if (4 == $this->majorversion) {
            switch ($frame_name) {
                ...
                case 'GRID':
                    ...
                    } elseif (in_array($frame_name . $source_data_array['ownerid'], $PreviousFrames, true)) {
                ...
                case 'GEOB':
                    ...
                    } elseif (in_array($frame_name . $source_data_array['description'], $PreviousFrames, true)) {
                ...
                case 'USER':
                    ...
                    } elseif (in_array($frame_name . $source_data_array['language'], $PreviousFrames, true)) {
                ...
                case 'COMM':
                    if (!isset($source_data_array['language'])) {
                        ...
                        $this->errors[] = '[description] not specified for ' . $frame_name;
                    } elseif (in_array($frame_name . $source_data_array['language'] . $source_data_array['description'], $PreviousFrames, true)) {
                ...
                case 'POPM':
                    ...
                    } elseif (in_array($frame_name . $source_data_array['email'], $PreviousFrames, true)) {
                ...
                case 'ASPI':
                case 'RGAD':
                    if (in_array($frame_name, $PreviousFrames, true)) {
                ...
                case 'LINK':
                    ...
                    } elseif (in_array($frame_name . $source_data_array['frameid'], $PreviousFrames, true)) {
                        $this->errors[] = 'Only one ' . $frame_name . ' tag allowed with the same FrameID (' . $source_data_array['frameid'] . ')';
                    } elseif (in_array($source_data_array['frameid'], $PreviousFrames, true)) {
                ...
                case 'SIGN':
                    ...
                    } elseif (in_array($frame_name . $source_data_array['ownerid'] . $source_data_array['data'], $PreviousFrames, true)) {
    ...
    public function GenerateID3v2Tag($noerrorsonly = true)
    {
        ...
        if (is_array($this->tag_data)) {
                        ...
                        if ($this->ID3v2FrameIsAllowed($frame_name, $source_data_array)) {
                            if ($frame_data = $this->GenerateID3v2FrameData($frame_name, $source_data_array)) {
                                ...
                                if ($this->majorversion >= 4) {
                                    ...
                                    if (mb_strlen($unsynchdata) != mb_strlen($frame_data)) {
                                        ...
                                        if (isset($TagUnsynchronisation) && false === $TagUnsynchronisation) {
                                    ...
                                    $frame_length = getid3_lib::BigEndian2String(mb_strlen($frame_data), 4, true);
                                } else {
                                    $frame_length = getid3_lib::BigEndian2String(mb_strlen($frame_data), 4, false);
                                }
            ...
            if (($this->majorversion <= 3) && $this->id3v2_use_unsynchronisation) {
                ...
                if (mb_strlen($unsynchdata) != mb_strlen($tagstring)) {
                    // unsynchronisation needed
            ...
            while ($this->paddedlength < (mb_strlen($tagstring) + getid3_id3v2::ID3v2HeaderLength($this->majorversion))) {
                $this->paddedlength += 1024;
            }

            $footer = false; // ID3v2 footers not yet supported in getID3()

            if (!$footer && ($this->paddedlength > (mb_strlen($tagstring) + getid3_id3v2::ID3v2HeaderLength($this->majorversion)))) {
                ...
                $tagstring .= @str_repeat("\x00", $this->paddedlength - mb_strlen($tagstring) - getid3_id3v2::ID3v2HeaderLength($this->majorversion));
            }

            if ($this->id3v2_use_unsynchronisation && ("\xFF" == mb_substr($tagstring, mb_strlen($tagstring) - 1, 1))) {
            ...
            $tagheader .= getid3_lib::BigEndian2String(mb_strlen($tagstring), 4, true);
    ...
    {
        if ('' == getid3_id3v2::LanguageLookup(mb_substr($pricestring, 0, 3), true)) {
            return false;
        } elseif (!$this->IsANumber(mb_substr($pricestring, 3), true)) {
            return false;
        }
    ...
    public function ID3v2IsValidETCOevent($eventid)
    {
        if (($eventid < 0) || ($eventid > 0xFF)) {
            ...
            return false;
        } elseif (($eventid >= 0x0E) && ($eventid <= 0x16) && (2 == $this->majorversion)) {
            // not defined in ID3v2.2

            return false;
        } elseif (($eventid >= 0x15) && ($eventid <= 0x16) && (3 == $this->majorversion)) {
    ...
    {
        if (($contenttype >= 0) && ($contenttype <= 8) && (4 == $this->majorversion)) {
            return true;
        } elseif (($contenttype >= 0) && ($contenttype <= 6) && (3 == $this->majorversion)) {
            return true;
        }
    ...
    {
        if (($channeltype >= 0) && ($channeltype <= 8) && (4 == $this->majorversion)) {
            return true;
        }
    ...
    public function ID3v2IsValidAPICimageformat($imageformat)
    {
        if ('-->' == $imageformat) {
            ...
            if ((3 == mb_strlen($imageformat)) && ($imageformat == mb_strtoupper($imageformat))) {
                return true;
            }
        } elseif ((3 == $this->majorversion) || (4 == $this->majorversion)) {
    ...
    {
        if ((mb_strlen($mimestring) >= 3) && (mb_strpos($mimestring, '/') > 0) && (mb_strpos($mimestring, '/') < (mb_strlen($mimestring) - 1))) {
            return true;
        }
    ...
    public function IsValidURL($url, $allowUserPass = false)
    {
        ...
        if ($parts = $this->safe_parse_url($url)) {
            if (('http' != $parts['scheme']) && ('https' != $parts['scheme']) && ('ftp' != $parts['scheme']) && ('gopher' != $parts['scheme'])) {
                return false;
    ...
    public function ID3v2ShortFrameNameLookup($majorversion, $long_description)
    {
        $long_description = str_replace(' ', '_', mb_strtolower(trim($long_description)));
        ...
        return @$ID3v2ShortFrameNameLookup[$majorversion][mb_strtolower($long_description)];
    }
}
