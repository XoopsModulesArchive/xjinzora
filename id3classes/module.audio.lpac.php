...
class getid3_lpac
{
    public function __construct(&$fd, &$ThisFileInfo)
    {
        ...
        if ('LPAC' != mb_substr($LPACheader, 0, 4)) {
        ...
        $ThisFileInfo['lpac']['file_version'] = getid3_lib::BigEndian2Int(mb_substr($LPACheader, 4, 1));

        $flags['audio_type'] = getid3_lib::BigEndian2Int(mb_substr($LPACheader, 5, 1));

        $ThisFileInfo['lpac']['total_samples'] = getid3_lib::BigEndian2Int(mb_substr($LPACheader, 6, 4));

        $flags['parameters'] = getid3_lib::BigEndian2Int(mb_substr($LPACheader, 10, 4));
        ...
        if ($ThisFileInfo['lpac']['flags']['fast_compress'] && (3 != $ThisFileInfo['lpac']['max_prediction_order'])) {
        ...
        switch ($ThisFileInfo['lpac']['file_version']) {
            case 6:
                ...
                if (20 != $ThisFileInfo['lpac']['quantization']) {
