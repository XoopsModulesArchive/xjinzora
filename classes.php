...
class jz_db
{
    ...
    public function select($query)
    {
        ...
        $queryCol = trim(mb_substr(str_replace('select where ', '', $query), 0, mb_strpos(str_replace('select where ', '', $query), ' =')));

        $queryVal = trim(mb_substr($query, mb_strpos($query, ' = ') + 3, mb_strlen($query)));
        ...
        for ($c = 0, $cMax = count($dataArray); $c < $cMax; $c++) {
            ...
            if (0 != $c) {
                ...
                for ($i = 0, $iMax = count($colArray); $i < $iMax; $i++) {
...
class SCXML
{
    ...
    public function set_host($sc_host)
    {
        $this->host = $sc_host;
    }

    public function set_port($sc_port)
    {
        $this->port = $sc_port;
    }

    public function set_password($sc_password)
    {
    ...
    public function retrieveXML()
    {
        ...
        if (!$sp) {
            ...
            fwrite($sp, "GET /admin.cgi?pass=$this->password&mode=viewxml HTTP/1.1\nUser-Agent:Mozilla\n\n");
            ...
            xml_set_elementHandler($this->parser, 'startElement', 'endElement');

            xml_set_character_dataHandler($this->parser, 'characterData');
    ...
    public function fetchMatchingArray($tag)
    {
        reset($this->xmlelem);
    ...
    public function fetchMatchingTag($tag)
    {
        reset($this->xmlelem);
