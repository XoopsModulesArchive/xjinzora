 ...
 */
global_include 'filesystem/media.php', 'filesystem/overrides.php', 'overrides.php';
global_include 'database/media.php', 'database/overrides.php', 'overrides.php';

function global_include($file, $include, $marker)
{
    // get the contents to include in a variable.
    if (!$inc = file_get_contents($include)) {
        die ("Could not open '$include'");
    }
    ...
    for ($i = 0, $iMax = count($lines); $i < $iMax; $i++) {
        if (preg_match("(\/\/ begin global_include: $marker)", $lines[$i])) {
            ...
            while (!(preg_match("(\/\/ end global_include: $marker)", $lines[$i]))) {
                if ($i >= count($lines)) {
                    die ('error: no end for global_include found.');
                }
                $i++;
            }
